...
def enum_value(enum, value):
    if enum.prefix is not None:
        return '{}_{}'.format(enum.prefix, value)
    return value
...
def write_value_strings(out, args):
    for vn, vs in values.items():
        out.write('const char *const {}_values[] = {{\n'.format(vn))
        out.write('{}\n   nullptr\n}};\n\n'.format(
            '\n'.join(['   "{}",'.format(x.lower()) for x in vs])))
...
def write_aliases(out, args):
    for enum in enums.values():
        ...
        for v in enum.values:
            out.write('constexpr auto {p}_{v} = {n}::{v};\n'.format(
...
def write_conversions(out, args):
    ...
    for enum in enums.values():
        ...
        out.write(header)
        out.write(
            'bool convert_string(const char *in, {} &out)\n'.format(
                enum.name))
        out.write(
            '{\n'
            '   if (false)\n'
    ...
    for enum in enums.values():
        out.write(header)
        out.write('const char *to_string({} val)\n'.format(enum.name))
        out.write('{\n'
                  '   switch (val)\n'
                  '   {\n')
...
def main():
    ...
    replacements = {
        '##BANNER##': write_banner,
        '##VALUE_STRINGS##': write_value_strings,
        '##ALIASES##': write_aliases,
        '##CONVERSIONS##': write_conversions,
    }
