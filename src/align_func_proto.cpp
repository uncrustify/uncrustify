...
void align_func_proto(size_t span)
{
   ...
   {
      char copy[1000];
      LOG_FMT(LAS, "%s(%d): orig line is %zu, orig_col is %zu, Text() is '%s', type is %s, level is %zu, brace_level is %zu\n",
                 ...
                 && options::align_single_line_func()))
      {
         ...
         LOG_FMT(LAS, "%s(%d): tmp->Text() is '%s', GetOrigLine() is %zu, orig_col is %zu, level is %zu, brace_level is %zu\n",
