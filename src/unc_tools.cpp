...
void dump_out(unsigned int type)
{
   ...
   if (D_file != nullptr)
   {
      for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = pc->next)
      {
         ...
         if (pc->orig_prev_sp != 0) fprintf(D_file, "  orig_prev_sp %u\n", pc->orig_prev_sp);
         if (pc->flags != 0) fprintf(D_file, "  flags %" PRIu64 "\n", pc->flags);
         if (pc->column != 0) fprintf(D_file, "  column %zu\n", pc->column);
         if (pc->column_indent != 0) fprintf(D_file, "  column_indent %zu\n", pc->column_indent);
         if (pc->nl_count != 0) fprintf(D_file, "  nl_count %zu\n", pc->nl_count);
         if (pc->level != 0) fprintf(D_file, "  level %zu\n", pc->level);
         if (pc->brace_level != 0) fprintf(D_file, "  brace_level %zu\n", pc->brace_level);
         if (pc->pp_level != 0) fprintf(D_file, "  pp_level %zu\n", pc->pp_level);
         if (pc->after_tab != 0) fprintf(D_file, "  after_tab %d\n", pc->after_tab);
         if (pc->type != CT_NEWLINE)
         {
