   ...
   for (Chunk const *pc = pc_sub; pc->IsNotNullChunk(); pc = pc->GetNext())
   {
      if (pc->GetOrigLine() == the_line_to_be_prot)
      {
            ...
            || partNumber == tokenCounter)
         {
            ...
            {
               LOG_FMT(LGUY, "<E_Token::CT_VBRACE_CLOSE>, ");
            }
            else if (pc->Is(E_Token::CT_VBRACE_OPEN))
            {
               LOG_FMT(LGUY, "<E_Token::CT_VBRACE_OPEN>, ");
            }
            else if (pc->Is(E_Token::CT_SPACE))
            {
               LOG_FMT(LGUY, "<E_Token::CT_SPACE>, ");
            }
            else if (pc->Is(E_Token::CT_IGNORED))
            {
