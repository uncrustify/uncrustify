...
static void detect_space_options()
{
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      if (pc->Is(E_Token::PTR_TYPE))
      {
         if (prev->Is(E_Token::PTR_TYPE))
         {
            vote_sp_between_ptr_star.vote(prev, pc);
         }
         else if (next->IsNot(E_Token::WORD))
         {
      ...
      {
         if (pc->GetParentType() == E_Token::FOR)
         {
            if (prev->Is(E_Token::SPAREN_OPEN))
            {
               // empty, ie for (;;)
               //               ^ is prev
               //                ^ is pc
            ...
            else if (next->Is(E_Token::SPAREN_CLOSE))
            {
               // empty, ie for (;;)
               //                 ^ is pc
               //                  ^ is next
            ...
            else if (prev->IsNot(E_Token::SEMICOLON))
            {
               // empty, ie for (; i < 8;)
               //                       ^ is pc
               // or
         ...
         else if (prev->Is(E_Token::VBRACE_OPEN))
         {
      ...
      {
         if (prev->Is(E_Token::ELSE))
         {
            vote_sp_else_brace.vote(prev, pc);
         }
         else if (prev->Is(E_Token::CATCH))
         {
            vote_sp_catch_brace.vote(prev, pc);
         }
         else if (prev->Is(E_Token::FINALLY))
         {
            vote_sp_catch_brace.vote(prev, pc);
         }
         else if (prev->Is(E_Token::TRY))
         {
            vote_sp_catch_brace.vote(prev, pc);
         }
         else if (prev->Is(E_Token::GETSET))
         {
            vote_sp_catch_brace.vote(prev, pc);
         }
      ...
      if (pc->Is(E_Token::BRACE_CLOSE))
      {
         ...
         else if (next->Is(E_Token::CATCH))
         {
            vote_sp_brace_catch.vote(pc, next);
         }
         else if (next->Is(E_Token::FINALLY))
         {
