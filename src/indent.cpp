...
void align_to_column(Chunk *pc, size_t column)
{
   ...
   do
   {
      ...
      if (  pc->IsComment()
         && pc->GetParentType() != E_Token::COMMENT_EMBED)
      {
...
static size_t token_indent(E_Token type)
{
   ...
   case E_Token::FOR:
   case E_Token::ELSE:  // wacky, but that's what is wanted
      return(4);
...
static size_t get_indent_first_continue(Chunk *pc)
{
   ...
   Chunk *continuation = pc->GetNextType(E_Token::NEWLINE, pc->GetLevel());
...
static Chunk *candidate_chunk_first_on_line(Chunk *pc)
{
   ...
   if (  options::indent_inside_ternary_operator()
      && (  first->Is(E_Token::QUESTION)
         || first->Is(E_Token::COND_COLON)))
   {
                                  ...
                                  bool from_keyword)
{
   ...
   if (  tmp->IsNotNullChunk()
      && tmp->GetType() == E_Token::OC_BLOCK_CARET)
   {
      caret_tmp = tmp;
   }
   else
   {
      caret_tmp = tmp->GetPrevType(E_Token::OC_BLOCK_CARET);
      tmp       = caret_tmp;
   }

   // If we still cannot find caret then return first chunk on the line
   if (  tmp->IsNullChunk()
      || tmp->IsNot(E_Token::OC_BLOCK_CARET))
   {
   ...
   {
      if (  tmp->IsNullChunk()
         || (  tmp->IsNot(E_Token::OC_MSG_NAME)
            && tmp->IsNot(E_Token::OC_MSG_FUNC)))
      {
         return(candidate_chunk_first_on_line(pc));
      }
      else
...
void indent_text()
{
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      if (  (pc->GetType() == E_Token::STRING_MULTI)
         ...
         && options::indent_col1_multi_string_literal())
      {
         ...
         if (  (str[0] == '@')
            && (pc->GetPrev()->GetType() == E_Token::NEWLINE))
         {
            indent_column_set(1);
      ...
      {
         LOG_FMT(LINDLINE, "%s(%d): orig line is %zu, E_Token::NL_CONT\n",
                 __func__, __LINE__, pc->GetOrigLine());
      }
      else
      {
      ...
      if (options::indent_func_def_force_col1())
      {
         if (!in_func_def)
         {
            ...
            if (  pc->GetParentType() == E_Token::FUNC_DEF
               || (  pc->Is(E_Token::COMMENT)
                  && next->IsNotNullChunk()
                  && next->GetParentType() == E_Token::FUNC_DEF))
            {
               in_func_def = true;
         ...
         else
         {
            ...
            if (  prev->Is(E_Token::BRACE_CLOSE)
      ...
      if (!in_preproc)
      {
               ...
               && frm.top().GetInPreproc())
         {
             ...
             */
            if (  type == E_Token::PP_ENDREGION
               && frm.top().GetOpenToken() == E_Token::PP_REGION_INDENT)
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
      ...
      else if (pc->Is(E_Token::PREPROC)) // #
      {
         // Close out PP_IF_INDENT before playing with the parse frames
         if (  frm.top().GetOpenToken() == E_Token::PP_IF_INDENT
            && (  pc->GetParentType() == E_Token::PP_ENDIF
               || pc->GetParentType() == E_Token::PP_ELSE))
         {
            LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            frm.pop(__func__, __LINE__, pc);
         ...
         if (  options::pp_region_indent_code()
            && pc->GetParentType() == E_Token::PP_REGION)
         {
            Chunk *next = pc->GetNext();
            ...
            next->SetType(E_Token::PP_REGION_INDENT);
            frm.push(next, __func__, __LINE__);
            next->SetType(E_Token::PP_REGION);
         ...
         if (  frm.top().GetOpenToken() == E_Token::CASE
         ...
         if (  options::pp_if_indent_code()
            && (  pc->GetParentType() == E_Token::PP_IF
               || pc->GetParentType() == E_Token::PP_ELSE))
         {
            Chunk *next = pc->GetNext();
            ...
            while (  preproc_next->IsNotNullChunk()
                  && preproc_next->IsNot(E_Token::NEWLINE))
            {
               if (  (preproc_next->Is(E_Token::BRACE_OPEN))
                  || (preproc_next->Is(E_Token::BRACE_CLOSE)))
               {
               ...
               else if (  (  preproc_next->Is(E_Token::FUNC_DEF)
                          && !options::pp_indent_func_def())
                       || (  preproc_next->Is(E_Token::CASE)
                          && !options::pp_indent_case())
                       || (  preproc_next->Is(E_Token::EXTERN)
            ...
            if (should_indent_preproc)
            {
               ...
               next->SetType(E_Token::PP_IF_INDENT);
               frm.push(next, __func__, __LINE__);
         ...
         if (options::indent_member_single())
         {
            ...
            {
               if (  frm.top().GetOpenToken() == E_Token::MEMBER
                  ...
                  && frm.top().GetOpenChunk() != frmbkup.top().GetOpenChunk())
               {
                  ...
                  if (tmp->IsNotNullChunk())
                  {
                     ...
                     else if (  tmp->Is(E_Token::FUNC_CALL)
                             || tmp->Is(E_Token::FPAREN_OPEN))
                     {
                        tmp = tmp->GetNextType(E_Token::FPAREN_CLOSE, tmp->GetLevel());
            ...
            {
               if (  frmbkup.top().GetOpenToken() == E_Token::MEMBER
                  && frm.top().GetOpenToken() == E_Token::MEMBER)
               {
         ...
         if (  pc->GetParentType() == E_Token::PP_DEFINE
            || pc->GetParentType() == E_Token::PP_UNDEF)
         {
         ...
         else if (  (  pc->GetParentType() == E_Token::PP_PRAGMA
                    || pc->GetParentType() == E_Token::PP_OTHER)
         ...
         else if (  pc->GetParentType() == E_Token::PP_INCLUDE
         ...
         {
            if (  (frm.prev().GetOpenToken() == E_Token::PP_REGION_INDENT)
               || (  (frm.prev().GetOpenToken() == E_Token::PP_IF_INDENT)
                  && (frm.top().GetOpenToken() != E_Token::PP_ENDIF)))
            {
            ...
            if (  pc->GetParentType() == E_Token::PP_REGION
               || pc->GetParentType() == E_Token::PP_ENDREGION)
            {
               log_rule_B("pp_indent_region");
               val = options::pp_indent_region();
               log_indent();
            }
            else if (  pc->GetParentType() == E_Token::PP_IF
                    || pc->GetParentType() == E_Token::PP_ELSE
                    || pc->GetParentType() == E_Token::PP_ENDIF)
            {
      ...
      do
      {
         ...
         if (frm.top().GetOpenLevel() >= pc->GetLevel())
         {
            ...
            if (  pc->Is(E_Token::VBRACE_CLOSE)
               && frm.top().GetOpenToken() == E_Token::VBRACE_OPEN)
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
            ...
            if (  options::indent_inside_ternary_operator()
               && (frm.top().GetOpenToken() == E_Token::COND_COLON)
               && (  pc->IsSemicolon()
                  || pc->Is(E_Token::COMMA)
                  || pc->Is(E_Token::OC_MSG_NAME)
                  || pc->Is(E_Token::SPAREN_CLOSE))) // Issue #1130, #1715
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
            ...
            // End any assign operations with a semicolon on the same level
            if (  pc->IsSemicolon()
               && (  (frm.top().GetOpenToken() == E_Token::IMPORT)
                  || (frm.top().GetOpenToken() == E_Token::USING)))
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
            ...
            if (  !token_used
               && (frm.top().GetOpenToken() == E_Token::MACRO_OPEN)
            ...
            if (  (  (frm.top().GetOpenToken() == E_Token::CLASS_COLON)
                  || (frm.top().GetOpenToken() == E_Token::CONSTR_COLON))
               && (  pc->Is(E_Token::BRACE_OPEN)
                  || pc->Is(E_Token::OC_END)
                  || pc->Is(E_Token::OC_SCOPE)
                  || pc->Is(E_Token::OC_PROPERTY)
                  || pc->Is(E_Token::TYPEDEF) // Issue #2675
                  || pc->Is(E_Token::MACRO_OPEN)
                  || pc->Is(E_Token::MACRO_CLOSE)
                  || (  language_is_set(lang_flag_e::LANG_OC)
                     && pc->IsComment()
                     && pc->GetParentType() == E_Token::COMMENT_WHOLE) // Issue #2675
                  || pc->IsSemicolon()))
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            ...
            if (  (frm.top().GetOpenToken() == E_Token::CLASS_COLON)
               ...
               && pc->GetParentType() == E_Token::OC_GENERIC_SPEC)
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
               ...
               && frm.top().GetOpenLevel() >= pc->GetLevel())
            {
               ...
               while (  next->IsNotNullChunk()
                     && (  (  next->Is(E_Token::BRACE_CLOSE)
                           && next->GetParentType() == E_Token::OC_AT)
               ...
               if (count > 0)
               {
                  while (count-- > 0)
                  {
                     if (frm.top().GetOpenToken() == E_Token::SQUARE_OPEN)
                     {
            ...
            if (  (frm.top().GetOpenToken() == E_Token::LAMBDA)
            ...
            if (  options::indent_access_spec_body()
               && (frm.top().GetOpenToken() == E_Token::ACCESS)
               && (  pc->Is(E_Token::BRACE_CLOSE)
                  || pc->Is(E_Token::ACCESS)))
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
            ...
            if (  pc->IsSemicolon()
               && (  (frm.top().GetOpenToken() == E_Token::RETURN)
                  || (frm.top().GetOpenToken() == E_Token::THROW)))
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
            ...
            if (  (frm.top().GetOpenToken() == E_Token::OC_SCOPE)
               && (  pc->IsSemicolon()
                  || pc->Is(E_Token::BRACE_OPEN)))
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
             ...
             * brace open
             */
            if (  (frm.top().GetOpenToken() == E_Token::TYPEDEF)
               && (  pc->IsSemicolon()
                  || pc->IsParenOpen()
                  || pc->Is(E_Token::BRACE_OPEN)))
            {
            ...
            if (  (frm.top().GetOpenToken() == E_Token::SQL_EXEC)
               && pc->IsSemicolon())
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            ...
            if (  (frm.top().GetOpenToken() == E_Token::CLASS)
               && (  pc->Is(E_Token::CLASS_COLON)
                  || pc->Is(E_Token::BRACE_OPEN)
                  || pc->IsSemicolon()))
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
            ...
            if (  options::indent_oc_inside_msg_sel()
               && (frm.top().GetOpenToken() != E_Token::SQUARE_OPEN)
               && frm.top().GetOpenLevel() >= pc->GetLevel()
               && (  pc->Is(E_Token::OC_MSG_FUNC)
                  || pc->Is(E_Token::OC_MSG_NAME))) // Issue #2658
            {
               LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
               frm.pop(__func__, __LINE__, pc);
            ...
            if (  (frm.top().GetOpenToken() == (pc->GetType() - 1))
               && (  pc->Is(E_Token::PAREN_CLOSE)
                  || pc->Is(E_Token::LPAREN_CLOSE)                     // Issue #3054
                  || pc->Is(E_Token::SPAREN_CLOSE)
                  || pc->Is(E_Token::FPAREN_CLOSE)
                  || pc->Is(E_Token::RPAREN_CLOSE)                     // Issue #3914
                  || pc->Is(E_Token::SQUARE_CLOSE)
                  || pc->Is(E_Token::ANGLE_CLOSE))
      ...
      if (  pc->Is(E_Token::BRACE_OPEN)
         && classFound)
      {
         LOG_FMT(LINDENT, "%s(%d): E_Token::BRACE_OPEN found, CLOSE IT\n",
                 __func__, __LINE__);
         LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                 __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
         classFound = false;
      ...
      const bool brace_indent = (  (  pc->Is(E_Token::BRACE_CLOSE)
                                   || pc->Is(E_Token::BRACE_OPEN))
                                ...
                                && (  !options::indent_braces_no_class()
                                   || pc->GetParentType() != E_Token::CLASS)
                                && (  !options::indent_braces_no_struct()
                                   || pc->GetParentType() != E_Token::STRUCT));
      ...
      if (pc->Is(E_Token::BRACE_CLOSE))
      {
         if (language_is_set(lang_flag_e::LANG_OC))
         {
            if (  frm.top().GetOpenToken() == E_Token::BRACE_OPEN
               && frm.top().GetOpenLevel() >= pc->GetLevel())
            {
               size_t count = 1;
               Chunk  *next = pc->GetNextNc();
      ...
      else if (pc->Is(E_Token::VBRACE_OPEN))
      {
         ...
         indent_column_set(frm.top().GetIndentTmp());
              ...
              && pc->GetNext()->IsNot(E_Token::NAMESPACE))
      {
         ...
         if (  !options::indent_macro_brace()
            && frm.prev().GetOpenToken() == E_Token::PP_DEFINE
                 ...
                 && pc->GetParentType() == E_Token::CPP_LAMBDA)
         {
            ...
            bool  enclosure = (  frm_prev->GetParentType() != E_Token::FUNC_DEF           // Issue #3407
            ...
            {
               if (!it->GetOpenChunk()->Is(E_Token::FPAREN_OPEN))
               {
                  continue;
               }
                 ...
                 && (pc->GetParentType() == E_Token::CPP_LAMBDA))
         {
            ...
            log_indent_tmp();
                 ...
                 && (  pc->GetParentType() == E_Token::LAMBDA
                    || pc->GetParentType() == E_Token::DELEGATE))
         {
                 ...
                 && pc->GetNextNc()->IsNewline())
         {
            ...
            if (  (pc->GetParentType() == E_Token::OC_BLOCK_EXPR)
         ...
         {
            if (frm.top().GetOpenChunk()->GetParentType() == E_Token::OC_BLOCK_EXPR)
            {
                  ...
                  || options::indent_oc_block_msg_xcode_style())
               {
                  ...
                  if (options::indent_oc_block_msg_xcode_style())
                  {
                     ...
                     if (  bbc_next_ncnl->GetType() == E_Token::OC_MSG_NAME
                        || bbc_next_ncnl->GetType() == E_Token::OC_MSG_FUNC)
                     {
            ...
            else if (  frm.top().GetOpenChunk()->GetType() == E_Token::BRACE_OPEN
                    && frm.top().GetOpenChunk()->GetParentType() == E_Token::OC_AT)
            {
                    ...
                    && !pc->TestFlags(PCF_ONE_LINER))
            {
               ...
               log_indent();
                    ...
                    && (  frm.prev().GetOpenToken() == E_Token::OC_MSG_FUNC
                       || frm.prev().GetOpenToken() == E_Token::OC_MSG_NAME)) // Issue #2658
            {
               log_rule_B("indent_oc_inside_msg_sel");
               // [Class Message:{<here>
            ...
            // Issue #3813
            else if (pc->TestFlags(PCF_OC_IN_BLOCK) && pc->GetParentType() == E_Token::SWITCH)
            {
               frm.top().SetIndent(frm.prev().GetIndentTmp());
            }
            else
         ...
         else if (  (  pc->GetParentType() == E_Token::BRACED_INIT_LIST
                    || (  !options::indent_compound_literal_return()
                       && pc->GetParentType() == E_Token::C_CAST))
                 && frm.prev().GetOpenToken() == E_Token::RETURN)
         {
            log_rule_B("indent_compound_literal_return");

            // we're returning either a c compound literal (E_Token::C_CAST) or a
            // C++11 initialization list (E_Token::BRACED_INIT_LIST), use indent from the return.
            if (frm.prev().GetIndentContinue())
            {
               frm.top().SetIndent(frm.prev().GetIndentTmp());
            }
         ...
         else
         {
            ...
            if (pc->GetParentType() == E_Token::SWITCH)
            {
            ...
            if (  pc->GetParentType() == E_Token::IF
               || pc->GetParentType() == E_Token::ELSE
               || pc->GetParentType() == E_Token::ELSEIF
               || pc->GetParentType() == E_Token::TRY
               || pc->GetParentType() == E_Token::CATCH
               || pc->GetParentType() == E_Token::DO
               || pc->GetParentType() == E_Token::WHILE
               || pc->GetParentType() == E_Token::USING_STMT
               || pc->GetParentType() == E_Token::SWITCH
               || pc->GetParentType() == E_Token::SYNCHRONIZED
            ...
            else if (pc->GetParentType() == E_Token::CASE)
            {
            ...
            else if (  pc->GetParentType() == E_Token::CLASS
            ...
            else if (pc->GetParentType() == E_Token::NAMESPACE)
            {
            ...
            else if (  pc->GetParentType() == E_Token::EXTERN
         ...
         else
         {
            ...
            if (  pc->GetParentType() == E_Token::BRACED_INIT_LIST
               && prev->Is(E_Token::BRACE_OPEN)
               && prev->GetParentType() == E_Token::BRACED_INIT_LIST)
            {
      ...
      else if (pc->Is(E_Token::SQL_END))
      {
         if (frm.top().GetOpenToken() == E_Token::SQL_BEGIN)
         {
            LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            frm.pop(__func__, __LINE__, pc);
      ...
      else if (  pc->Is(E_Token::SQL_BEGIN)
              || pc->Is(E_Token::MACRO_OPEN)
              || (  pc->Is(E_Token::CLASS)
      ...
      else if (pc->Is(E_Token::SQL_EXEC))
      {
         frm.push(pc, __func__, __LINE__);
      ...
      else if (pc->Is(E_Token::MACRO_ELSE))
      {
         if (frm.top().GetOpenToken() == E_Token::MACRO_OPEN)
         {
            indent_column_set(frm.prev().GetIndent());
         }
      }
      else if (pc->Is(E_Token::CASE))
      {
      ...
      else if (pc->Is(E_Token::BREAK))
      {
         Chunk *prev = pc->GetPrevNcNnl();

         if (  prev->Is(E_Token::BRACE_CLOSE)
            && prev->GetParentType() == E_Token::CASE)
         {
      ...
      else if (  pc->Is(E_Token::CLASS_COLON)
              || pc->Is(E_Token::CONSTR_COLON))
      {
         ...
         else if (pc->Is(E_Token::CONSTR_COLON))
         {
            ...
            if (options::indent_constr_colon())
            {
              ...
              && (  pc->GetParentType() == E_Token::ASM
                 || (  pc->GetPrevNcNnl()->IsNotNullChunk()
                    && pc->GetPrevNcNnl()->GetType() == E_Token::ASM))
              && options::indent_ignore_asm_block())
      {
         ...
         reindent_line(pc, indent_column);
                 ...
                 || pc->Is(E_Token::LPAREN_OPEN)                     // Issue #3054
                 || pc->Is(E_Token::SPAREN_OPEN)
                 || pc->Is(E_Token::FPAREN_OPEN)
                 || pc->Is(E_Token::RPAREN_OPEN)                     // Issue #3914
                 || pc->Is(E_Token::SQUARE_OPEN)
                 || pc->Is(E_Token::ANGLE_OPEN))
              && (  !pc->TestFlags(PCF_CONT_LINE)
                 || options::indent_continue() >= 0))
      {
         ...
         if (  prev_ncnl->IsNotNullChunk()
            && (  prev_ncnl->Is(E_Token::MACRO_NO_INDENT)
               || prev_ncnl->Is(E_Token::MACRO_NO_FMT_ARGS)))
         {
         ...
         else
         {
            ...
            if (options::indent_bool_nested_all())
            {
               ...
               if (prev->Is(E_Token::BOOL) && (prev->GetPrevNc()->IsNewline() || prev->GetNextNc()->IsNewline()))
               {
               ...
               if (next->Is(E_Token::BOOL) && (next->GetNextNc()->IsNewline() || next->GetPrevNc()->IsNewline()))
               {
                  indent_bool_more = true;
               }
            }
            ...
            if (  options::indent_inside_ternary_operator()
               && (  pc->Is(E_Token::FPAREN_OPEN)
                  || pc->Is(E_Token::PAREN_OPEN))
               && frm.size() > 2
               && (  frm.prev().GetOpenToken() == E_Token::QUESTION
                  || frm.prev().GetOpenToken() == E_Token::COND_COLON)
               && !options::indent_align_paren())
            {
               frm.top().SetIndent(frm.prev().GetIndentTmp() + indent_size);
               log_indent();
               ...
               log_indent_tmp();
                    ...
                    && (  (  options::indent_func_call_param()
                       ...
                       || (  options::indent_func_ctor_var_param()
                          && pc->GetParentType() == E_Token::FUNC_CTOR_VAR)
                          ...
                          && pc->GetOrigCol() > options::indent_func_def_param_paren_pos_threshold())))
            {
               ...
               while (  (  (  idx > 0
                           && frm.at(idx).GetOpenToken() != E_Token::BRACE_OPEN
                           && frm.at(idx).GetOpenToken() != E_Token::VBRACE_OPEN
                           && frm.at(idx).GetOpenToken() != E_Token::PAREN_OPEN
                           && frm.at(idx).GetOpenToken() != E_Token::FPAREN_OPEN
                           && frm.at(idx).GetOpenToken() != E_Token::RPAREN_OPEN                     // Issue #3914
                           && frm.at(idx).GetOpenToken() != E_Token::SPAREN_OPEN
                           && frm.at(idx).GetOpenToken() != E_Token::SQUARE_OPEN
                           && frm.at(idx).GetOpenToken() != E_Token::ANGLE_OPEN
                           && frm.at(idx).GetOpenToken() != E_Token::CASE
                           && frm.at(idx).GetOpenToken() != E_Token::MEMBER
                           && frm.at(idx).GetOpenToken() != E_Token::QUESTION
                           && frm.at(idx).GetOpenToken() != E_Token::COND_COLON
                           && frm.at(idx).GetOpenToken() != E_Token::LAMBDA
                           && frm.at(idx).GetOpenToken() != E_Token::ASSIGN_NL)
                        || frm.at(idx).GetOpenChunk()->IsOnSameLine(frm.top().GetOpenChunk()))
                     && (  frm.at(idx).GetOpenToken() != E_Token::CLASS_COLON
                        && frm.at(idx).GetOpenToken() != E_Token::CONSTR_COLON
                        && !(  frm.at(idx).GetOpenToken() == E_Token::LAMBDA
                            && frm.at(idx).GetOpenChunk()->GetPrevNc()->GetType() == E_Token::NEWLINE)))
               {
                  if (idx == 0)
                  {
               ...
               frm.top().SetIndentTab(frm.top().GetIndent());
                    ...
                    && frm.size() > 2
                    && (  frm.prev().GetOpenToken() == E_Token::OC_MSG_FUNC
                       || frm.prev().GetOpenToken() == E_Token::OC_MSG_NAME)
                       ...
                       && !options::indent_square_nl()))
            {
                     ...
                     || options::indent_continue() >= 0))
               {
                  ...
                  if (  (frm.prev().GetOpenToken() == E_Token::ASSIGN)
                     || (frm.prev().GetOpenToken() == E_Token::RETURN))
                  {
                     sub = 3;
                  }
                  ...
                  if (!options::indent_align_paren())
                  {
                     ...
                     if (  (  frm.at(sub + 1).GetOpenToken() == E_Token::CLASS_COLON
                           || frm.at(sub + 1).GetOpenToken() == E_Token::CONSTR_COLON)
                        && (frm.at(sub + 1).GetOpenChunk()->GetPrev()->Is(E_Token::NEWLINE)))
                     {
                        sub = sub + 1;
                     }
                  }
            ...
            if (  pc->GetParentType() != E_Token::OC_AT
               ...
               && !skipped)
            {
               ...
               if (  pc->GetLevel() == pc->GetBraceLevel()
                  && !options::indent_ignore_first_continue()
                  && (  pc->Is(E_Token::FPAREN_OPEN)
                     || pc->Is(E_Token::RPAREN_OPEN)                   // Issue #1170
                     || pc->Is(E_Token::SPAREN_OPEN)
                     || (  pc->Is(E_Token::SQUARE_OPEN)
                        && pc->GetParentType() != E_Token::OC_MSG)
                     || pc->Is(E_Token::ANGLE_OPEN)))                  // Issue #1170
               {
                  log_rule_B("use_indent_continue_only_once");
                 ...
                 || language_is_set(lang_flag_e::LANG_CPP)))
      {
         ...
         if (pc->GetPrev()->IsNewline())
         {
            if (  pc->Is(E_Token::MEMBER)                           // Issue #2890
               && language_is_set(lang_flag_e::LANG_CPP))
            {
         ...
         //check for the series of E_Token::member chunks else pop it.
         Chunk *tmp = pc->GetNextNcNnlNpp();

         if (tmp->IsNotNullChunk())
         {
            if (tmp->Is(E_Token::FUNC_CALL))
            {
               tmp = tmp->GetNextType(E_Token::FPAREN_CLOSE, tmp->GetLevel());
               tmp = tmp->GetNextNcNnlNpp();
            }
            else if (  tmp->Is(E_Token::WORD)
                    || tmp->Is(E_Token::TYPE))
            {
               tmp = tmp->GetNextNcNnlNpp();
            }
                 ...
                 && language_is_set(lang_flag_e::LANG_CS)))
      {
         ...
         if (  pc->Is(E_Token::ASSIGN)
            && pc->GetPrev()->IsNewline())
         {
            if (frm.top().GetOpenToken() == E_Token::ASSIGN_NL)
            {
               frm.top().SetIndentTmp(frm.top().GetIndent());
            }
            else
         ...
         if (next->IsNotNullChunk())
         {
             ...
             */
            if (  frm.top().GetOpenToken() == E_Token::ASSIGN
            ...
            if (  pc->Is(E_Token::ASSIGN)
               && pc->GetPrev()->IsNewline())
            {
               frm.top().SetOpenToken(E_Token::ASSIGN_NL);
            }
            log_rule_B("indent_continue");
      ...
      else if (  pc->Is(E_Token::OC_SCOPE)
      ...
      else if (pc->Is(E_Token::C99_MEMBER))
      {
         // nothing to do
      }
      else if (pc->Is(E_Token::WHERE_SPEC))
      {
         ...
         if (  pc->GetParentType() == E_Token::FUNC_DEF
            || pc->GetParentType() == E_Token::FUNC_PROTO
            || (  pc->GetParentType() == E_Token::STRUCT
               && frm.top().GetOpenToken() != E_Token::CLASS_COLON))
         {
            indent_column_set(frm.top().GetIndent() + 4);
              ...
              && (  pc->Is(E_Token::QUESTION)
                 || pc->Is(E_Token::COND_COLON))) // Issue #1130, #1715
      {
         log_rule_B("indent_inside_ternary_operator");

         // Pop any colons before because they should already be processed
         while (  pc->Is(E_Token::COND_COLON)
               && frm.top().GetOpenToken() == E_Token::COND_COLON)
         {
            frm.pop(__func__, __LINE__, pc);
         }
         ...
         if (  options::indent_inside_ternary_operator()
            ...
            && frm.top().GetOpenToken() == E_Token::QUESTION)
         {
            LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            frm.pop(__func__, __LINE__, pc);
      ...
      else if (  pc->Is(E_Token::LAMBDA)
              && (  language_is_set(lang_flag_e::LANG_CS)
                 || language_is_set(lang_flag_e::LANG_JAVA))
              && pc->GetNextNcNnlNpp()->IsNot(E_Token::BRACE_OPEN)
              && options::indent_cs_delegate_body())
      {
         ...
         log_indent_tmp();
              ...
              && (  pc->Is(E_Token::OC_MSG_FUNC)
                 || pc->Is(E_Token::OC_MSG_NAME))
              && pc->GetNextNcNnl()->Is(E_Token::OC_COLON)) // Issue #2658
      {
         log_rule_B("indent_oc_inside_msg_sel");
...
// #define ANYTHING_ELSE
      ...
      if (  options::indent_shift() == 1
         ...
         && pc->GetParentType() != E_Token::OPERATOR
         && !pc->IsComment()
         && pc->IsNot(E_Token::BRACE_OPEN)
         && !pc->IsEmptyText())
      {
         if (pc->Is(E_Token::SHIFT))
         {
            in_shift      = true;
            current_shift = pc;
         }
            ...
            && in_shift)
         {
            ...
            {
               if (  frm.at(i).GetOpenToken() == E_Token::RETURN
                  || frm.at(i).GetOpenToken() == E_Token::ASSIGN)
               {
         ...
         && pc->TestFlags(PCF_VAR_1ST_DEF))
      {
         ...
         else
         {
            ...
            //if (tmp->Is(E_Token::DC_MEMBER))
            //{
            //   // look for a type
            //   Chunk *tmp2 = tmp->GetPrevNc();
            //   if (tmp2->Is(E_Token::TYPE))
            //   {
      ...
      if (  pc->IsSemicolon()
         || (  pc->Is(E_Token::BRACE_OPEN)
            && (  pc->GetParentType() == E_Token::FUNCTION
               || pc->GetParentType() == E_Token::CLASS))) //Issue #3576
      {
         vardefcol = 0;
      }
         ...
         && (pc->Len() != 0))
      {
         ...
         else if (  pc->GetParentType() == E_Token::SQL_EXEC
         ...
         else if (  pc->Is(E_Token::NAMESPACE)
                 && options::indent_namespace()
                 && options::indent_namespace_single_indent()
                 && frm.top().GetNsCount())
         {
                 ...
                 && options::indent_align_string())
         {
            ...
            else
            {
               ...
               while (  tmp->GetPrev()->IsNotNullChunk()
                     && (  tmp->GetPrev()->Is(E_Token::WORD)
                        || tmp->GetPrev()->Is(E_Token::STRING)))
               {
                  tmp = tmp->GetPrev();
               }
            ...
            if (frm.lastPopped().GetOpenToken() == E_Token(pc->GetType() - 1))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     if (pc->GetPrev()->GetType() == E_Token::NEWLINE)
                     {
                        LOG_FMT(LINDLINE, "%s(%d): orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                                __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
                        ...
                        if (searchNext->GetType() == E_Token::NOEXCEPT)
                        {
                           searchNext = searchNext->GetNext();
                        }

                        if (  searchNext->GetType() == E_Token::SEMICOLON
                           || searchNext->GetType() == E_Token::MEMBER            // Issue #2582
                           || searchNext->GetType() == E_Token::NEWLINE)
                        {
                           LOG_FMT(LINDLINE, "%s(%d):\n", __func__, __LINE__);
                           search = search->GetOpeningParen();

                           if (  options::indent_oc_inside_msg_sel()
                              && search->GetPrevNcNnl()->Is(E_Token::OC_COLON)
                              && (  frm.top().GetOpenToken() == E_Token::OC_MSG_FUNC
                                 || frm.top().GetOpenToken() == E_Token::OC_MSG_NAME)) // Issue #2658
                           {
                              log_rule_B("indent_oc_inside_msg_sel");
                              // [Class Message:(...)<here>
                              indent_column_set(frm.top().GetOpenChunk()->GetColumn());
                                   ...
                                   && (  frm.top().GetOpenToken() == E_Token::QUESTION
                                      || frm.top().GetOpenToken() == E_Token::COND_COLON)) // Issue #1130, #1715
                           {
                              log_rule_B("indent_inside_ternary_operator");
                              indent_column_set(frm.top().GetIndent());
                           }
         ...
         else if (pc->Is(E_Token::ASSIGN))
         {
            ...
            reindent_line(pc, indent_column);
                 ...
                 && prev->Is(E_Token::COND_COLON))
         {
            log_rule_B("indent_ternary_operator");
            Chunk *tmp = prev->GetPrevType(E_Token::QUESTION);
         ...
         else
         {
            ...
            if (ttidx > 0)
            {
               ...
               if ((frm.at(ttidx).GetOpenChunk())->GetParentType() == E_Token::FUNC_CALL)
               {
            ...
            if (  use_indent
               && pc->IsNot(E_Token::PP_IGNORE)) // Leave indentation alone for PP_IGNORE tokens
            {
               log_rule_B("pos_conditional");

               if (  (  pc->Is(E_Token::QUESTION)      // Issue #2101
                     || pc->Is(E_Token::COND_COLON))   // Issue #2101
               ...
               else if (pc->Is(E_Token::BREAK))
               {
                  ...
                  if (  options::indent_switch_break_with_case()
                     && pc->GetTypeOfParent() == E_Token::SWITCH)
                  {
                     // look for a case before Issue #2735
                     Chunk *whereIsCase = pc->GetPrevType(E_Token::CASE, pc->GetLevel());
               ...
               else if (  pc->Is(E_Token::MEMBER)                      // Issue #2890
                       && language_is_set(lang_flag_e::LANG_CPP))
               {
                  ...
                  else
                  {
                     ...
                     if (token_before->Is(E_Token::ASSIGN))
                     {
                     ...
                     else if (token_before->Is(E_Token::BRACE_OPEN))
                     {
                     ...
                     else if (token_before->Is(E_Token::RETURN))
                     {
         ...
         if (pc->TestFlags(PCF_VAR_TYPE))
         {
            if (  !frm.top().GetNonVardef()
               && (frm.top().GetOpenToken() == E_Token::BRACE_OPEN))
            {
         ...
         || pc->Is(E_Token::COMMENT_CPP))
      {
          ...
          */
         if (  pc->Is(E_Token::NL_CONT)
            && frm.top().GetIndentTmp() <= indent_size
            && frm.top().GetOpenToken() != E_Token::PP_DEFINE)
         {
      ...
      if (  pc->Is(E_Token::CLASS)
         ...
         && !classFound)
      {
         LOG_FMT(LINDENT, "%s(%d): orig line is %zu, E_Token::CLASS found, OPEN IT\n",
                 __func__, __LINE__, pc->GetOrigLine());
         frm.push(pc, __func__, __LINE__);
      ...
      if (pc->Is(E_Token::SPACE))                       // Issue #3710
      {
         pc = pc->GetNext();
      }
      LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
...
static bool is_end_of_assignment(Chunk *pc, const ParsingFrame &frm)
{
   return(  (  frm.top().GetOpenToken() == E_Token::ASSIGN_NL
            || frm.top().GetOpenToken() == E_Token::MEMBER
            || frm.top().GetOpenToken() == E_Token::ASSIGN)
         && (  pc->IsSemicolon()
            || pc->Is(E_Token::COMMA)
            || pc->Is(E_Token::BRACE_OPEN)
            || pc->Is(E_Token::SPAREN_CLOSE)
...
void indent_preproc()
{
   ...
   for (Chunk *pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext())
   {
      ...
      bool at_file_level = pc->GetBraceLevel() <= ((pc->GetParentType() == E_Token::PP_DEFINE) ? 1 : 0);
         ...
         && pc->GetParentType() != E_Token::PP_ENDREGION)
      {
         ...
         if (  !options::pp_define_at_level()
            || pc->GetParentType() != E_Token::PP_DEFINE)
         {
