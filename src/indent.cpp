...
static void indent_pse_pop(parse_frame_t &frm, chunk_t *pc)
{
   ...
   if (frm.pse_tos > 0)
   {
      if (pc != NULL)
      {
         LOG_FMT(LINDPSE, "%4zu] (pp=%d) CLOSE [%zu,%s] on %s, started on line %d, level=%zu/%zu\n",
   ...
   while (pc != NULL)
   {
      ...
      if (!chunk_is_newline(pc) && !chunk_is_comment(pc) && log_sev_on(LINDPC))
      {
         ...
         for (int ttidx = frm.pse_tos; ttidx > 0; ttidx--)
         {
            ...
            LOG_FMT(LINDPC, "     [%d %zu:%zu %s %s/%s tmp=%zu ind=%d bri=%d tab=%d cont=%d lvl=%zu blvl=%zu]\n",
                    ttidx,
      ...
      else if ((pc->type == CT_OC_SCOPE) || (pc->type == CT_TYPEDEF))
      {
         ...
         frm.pse[frm.pse_tos].indent     = frm.pse[frm.pse_tos - 1].indent;
         frm.pse[frm.pse_tos].indent_tmp = frm.pse[frm.pse_tos].indent;
         LOG_FMT(LINDLINE, "%s(%d): .indent=%d, .indent_tmp=%zu\n",
...
void indent_preproc(void)
{
   ...
   for (chunk_t *pc = chunk_get_head(); pc != NULL; pc = chunk_get_next(pc))
   {
      ...
      if ((cpd.settings[UO_pp_space].a != AV_IGNORE) && (next != NULL))
      {
         ...
         {
            int mult = cpd.settings[UO_pp_space_count].u;
