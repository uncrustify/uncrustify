...
static bool d_parse_string(tok_ctx &ctx, chunk_t &pc)
{
   ...
   if (ch == '\\')
   {
      ...
      if (pc.str.size() > 1)
      {
         set_chunk_type(&pc, CT_STRING);                    // Issue #2567
         return(true);
      }
      ctx.restore();
   }
...
static bool parse_comment(tok_ctx &ctx, chunk_t &pc)
{
   ...
   if (ch == '/')
   {
      set_chunk_type(&pc, CT_COMMENT_CPP);                    // Issue #2567
   ...
   {
      set_chunk_type(&pc, CT_COMMENT);                    // Issue #2567
      d_level++;
      ...
      while (d_level > 0 && ctx.more())
      {
         ...
         if ((ch == '\n') || (ch == '\r'))
         {
            set_chunk_type(&pc, CT_COMMENT_MULTI);                    // Issue #2567
            pc.nl_count++;
...
static bool parse_code_placeholder(tok_ctx &ctx, chunk_t &pc)
{
   ...
   while (ctx.more())
   {
      ...
      if ((last2 == '#') && (last1 == '>'))
      {
         set_chunk_type(&pc, CT_WORD);                    // Issue #2567
         return(true);
      }
   }
   ctx.restore();
...
static bool parse_number(tok_ctx &ctx, chunk_t &pc)
{
   ...
   if ((ctx.peek() == '.') && (ctx.peek(1) != '.'))
   {
      ...
      {
         ctx.restore(ss);
         set_chunk_type(&pc, CT_NUMBER);                    // Issue #2567
         return(true);
      }
      else
      {
   ...
   set_chunk_type(&pc, is_float ? CT_NUMBER_FP : CT_NUMBER);                    // Issue #2567
...
static bool parse_string(tok_ctx &ctx, chunk_t &pc, size_t quote_idx, bool allow_escape)
{
   ...
   while (ctx.more())
   {
      ...
      {
         pc.nl_count++;
         set_chunk_type(&pc, CT_STRING_MULTI);                    // Issue #2567
         escaped = false;
         continue;
      }
...
static bool parse_word(tok_ctx &ctx, chunk_t &pc, bool skipcheck)
{
   ...
   if (cpd.in_preproc == CT_PP_DEFINE && cpd.preproc_ncnl_count == 1)
   {
      if (ctx.peek() == '(')
      {
         set_chunk_type(&pc, CT_MACRO_FUNC);                    // Issue #2567
      }
      else
      {
         set_chunk_type(&pc, CT_MACRO);                    // Issue #2567
   ...
   else
   {
      ...
      {
         set_chunk_type(&pc, CT_ANNOTATION);                    // Issue #2567
      }
      else
      {
         // Turn it into a keyword now
         // Issue #1460 will return "COMMENT_CPP"
         set_chunk_type(&pc, find_keyword_type(pc.text(), pc.str.size()));   // Issue #2567
...
static bool extract_attribute_specifier_sequence(tok_ctx &ctx, chunk_t &pc, size_t length)
{
   ...
   set_chunk_type(&pc, CT_ATTRIBUTE);                    // Issue #2567
   return(true);
...
{
   pc.str.clear();
   set_chunk_type(&pc, tt);                    // Issue #2567
...
static bool parse_ignored(tok_ctx &ctx, chunk_t &pc)
{
   ...
   if (!ontext.empty() && pc.str.find(ontext.c_str()) < 0)
   {
      set_chunk_type(&pc, CT_IGNORED);                    // Issue #2567
      return(true);
   }
   ctx.restore();
...
static bool parse_next(tok_ctx &ctx, chunk_t &pc)
{
   ...
   if (cpd.in_preproc > CT_PP_BODYCHUNK && cpd.in_preproc <= CT_PP_OTHER)
   {
      ...
      set_chunk_type(&pc, CT_PREPROC_BODY);                    // Issue #2567
      size_t last = 0;
...
//ctx.restore(ctx.c);
   ...
   set_chunk_type(&pc, CT_UNKNOWN);                    // Issue #2567
   pc.str.append(ctx.get());
