...
bool newlines_if_for_while_switch(Chunk *start, iarf_e nl_opt)
{
   ...
   {
      Chunk *close_paren = pc->GetNextType(E_Token::SPAREN_CLOSE, pc->GetLevel());
      Chunk *brace_open  = close_paren->GetNextNcNnl();

      if (  (  brace_open->Is(E_Token::BRACE_OPEN)
            || brace_open->Is(E_Token::VBRACE_OPEN))
         && one_liner_nl_ok(brace_open))
      {
         ...
         else
         {
            ...
            pc = brace_open->GetNextType(E_Token::BRACE_CLOSE, brace_open->GetLevel());
...
void newlines_if_for_while_switch_post_blank_lines(Chunk *start, uncrustify::iarf_e nl_opt)
{
    ...
    */
   if (start->Is(E_Token::DO))
   {
      // point to the next semicolon
      if ((pc = pc->GetNextType(E_Token::SEMICOLON, start->GetLevel()))->IsNullChunk())
      {
         return;
      }
      LOG_FMT(LNEWLINE, "%s(%d): text is '%s', type %s, orig line %zu, orig col %zu\n",
   ...
   bool isVBrace = (pc->Is(E_Token::VBRACE_CLOSE));
   ...
   if (nl_opt & IARF_ADD)
   {
      ...
      do
      {
         ...
         if (next->IsNot(E_Token::VBRACE_CLOSE))
         {
            break;
         }
