...
void newlines_brace_pair(Chunk *br_open)
{
   ...
   if (  parent == E_Token::IF
      || parent == E_Token::ELSEIF
      || parent == E_Token::ELSE)
   {
   ...
   if (br_open->GetParentType() == E_Token::ASSIGN)
   {
   ...
   if (  br_open->GetParentType() == E_Token::OC_MSG_DECL
      || br_open->GetParentType() == E_Token::FUNC_DEF
      || br_open->GetParentType() == E_Token::FUNC_CLASS_DEF
      || br_open->GetParentType() == E_Token::OC_CLASS
      || br_open->GetParentType() == E_Token::CS_PROPERTY
      || br_open->GetParentType() == E_Token::CPP_LAMBDA
      || br_open->GetParentType() == E_Token::FUNC_CALL
      || br_open->GetParentType() == E_Token::FUNC_CALL_USER)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            val = ((br_open->GetParentType() == E_Token::CS_PROPERTY) ?
   ...
   if (br_open->GetNextNnl()->Is(E_Token::BRACE_CLOSE))
   {
   ...
   if (options::eat_blanks_after_open_brace())
   {
      if (next->IsNewline())
      {
         ...
         if (  options::nl_inside_empty_func() > 0
            && br_open->GetNextNnl()->Is(E_Token::BRACE_CLOSE)
            && (  br_open->GetParentType() == E_Token::FUNC_CLASS_DEF
               || br_open->GetParentType() == E_Token::FUNC_DEF))
         {
   ...
   Chunk *br_close = br_open->GetNextType(E_Token::BRACE_CLOSE, br_open->GetLevel());
   ...
   {
      /*
       * If the open brace hits a E_Token::NEWLINE, E_Token::NL_CONT, E_Token::COMMENT_MULTI, or
       * E_Token::COMMENT_CPP without hitting anything other than E_Token::COMMENT, then
       * there should be a newline before the close brace.
       */
      Chunk *pc = br_open->GetNext();

      while (pc->Is(E_Token::COMMENT))
      {
         pc = pc->GetNext();
      }
