...
void newline_func_def_or_call(Chunk *start)
{
   ...
   bool is_def = (start->GetParentType() == E_Token::FUNC_DEF)
                 || start->GetParentType() == E_Token::FUNC_CLASS_DEF;
   bool is_call = (start->GetParentType() == E_Token::FUNC_CALL)
                  || start->GetParentType() == E_Token::FUNC_CALL_USER;
   ...
   else
   {
      ...
      if (prev->IsNot(E_Token::ACCESS_COLON))
      {
         Chunk *tmp;

         if (prev->Is(E_Token::OPERATOR))
         {
            tmp  = prev;
         ...
         if (tmp_next->IsNot(E_Token::FUNC_CLASS_DEF))
         {
            Chunk  *closing = tmp->GetClosingParen();
            Chunk  *brace   = closing->GetNextNcNnl();
            iarf_e a;                                            // Issue #2561

            if (  tmp->GetParentType() == E_Token::FUNC_PROTO
               || tmp->GetParentType() == E_Token::FUNC_CLASS_PROTO)
            {
               // proto
               ...
               && prev->IsNotNullChunk())
            {
               ...
               if (  !prev->IsBraceClose()
                  ...
                  && prev->IsNot(E_Token::ACCESS_COLON)
                     ...
                     // return type syntax
                  && (tmp_next->IsNot(E_Token::OPERATOR) ? true : prev->IsTypeDefinition()))
               {
                  newline_iarf(prev, a);
               }
            }
        ...
        pc = pc->GetNextNcNnl())
   {
         ...
         && (pc->GetLevel() == (start->GetLevel() + 1)))
      {
         ...
         else // start->GetParentType() == E_Token::FUNC_DECL
         {
   ...
   if (pc->Is(E_Token::FPAREN_CLOSE))
   {
      ...
      if (  prev->IsNot(E_Token::FPAREN_OPEN)
         && !is_call)
      {
...
void newline_func_multi_line(Chunk *start)
{
   ...
   if (  start->GetParentType() == E_Token::FUNC_DEF
      || start->GetParentType() == E_Token::FUNC_CLASS_DEF)
   {
   ...
   else if (  start->GetParentType() == E_Token::FUNC_CALL
           || start->GetParentType() == E_Token::FUNC_CALL_USER)
   {
      ...
      && start->IsNewlineBetween(pc))
   {
      ...
      bool  has_leading_closure = (  start_next->GetParentType() == E_Token::OC_BLOCK_EXPR
                                  || start_next->GetParentType() == E_Token::CPP_LAMBDA
                                  || start_next->Is(E_Token::BRACE_OPEN));

      Chunk *prev_end            = pc->GetPrevNcNnl();
      bool  has_trailing_closure = (  prev_end->GetParentType() == E_Token::OC_BLOCK_EXPR
                                   || prev_end->GetParentType() == E_Token::CPP_LAMBDA
                                   || prev_end->Is(E_Token::BRACE_OPEN));
               ...
               && (pc->GetLevel() == (start->GetLevel() + 1)))
            {
               ...
               if (!pc->GetNext()->IsNewline())
               {
                  ...
                  if (options::nl_func_call_args_multi_line_ignore_closures())
                  {
                     ...
                     if (!(  (  prev_comma->GetParentType() == E_Token::OC_BLOCK_EXPR
                             || prev_comma->GetParentType() == E_Token::CPP_LAMBDA
                             || prev_comma->Is(E_Token::BRACE_OPEN))
                          || (  after_comma->GetParentType() == E_Token::OC_BLOCK_EXPR
                             || after_comma->GetParentType() == E_Token::CPP_LAMBDA
                             || after_comma->Is(E_Token::BRACE_OPEN))))
                     {
