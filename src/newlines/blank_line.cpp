...
void do_blank_lines()
{
   ...
   for (Chunk *pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext())
   {
         ...
         && next->Is(E_Token::COMMENT_MULTI))
      {
         ...
         if (  (  prev->IsNullChunk()
               || (  prev->IsNot(E_Token::BRACE_OPEN)
                  && prev->IsNot(E_Token::VBRACE_OPEN)
                  && prev->IsNot(E_Token::CASE_COLON)))
            && pcmt->IsNot(E_Token::COMMENT_MULTI))    // Issue #2383
         {
         ...
         && next->Is(E_Token::COMMENT))
      {
         ...
         if (  (  prev->IsNullChunk()
               || (  prev->IsNot(E_Token::BRACE_OPEN)
                  && prev->IsNot(E_Token::VBRACE_OPEN)
                  && prev->IsNot(E_Token::CASE_COLON)))
            && pcmt->IsNot(E_Token::COMMENT))    // Issue #2383
         {
         ...
         && next->Is(E_Token::COMMENT_CPP))
      {
         ...
         if (  (  prev->IsNullChunk()
               || (  prev->IsNot(E_Token::BRACE_OPEN)
                  && prev->IsNot(E_Token::VBRACE_OPEN)
                  && prev->IsNot(E_Token::CASE_COLON)))
            && pcmt->IsNot(E_Token::COMMENT_CPP))    // Issue #2383
         {
      ...
      if (  (  prev->Is(E_Token::SEMICOLON)
            || prev->Is(E_Token::BRACE_CLOSE))
         && (  prev->GetParentType() == E_Token::CLASS
            || prev->GetParentType() == E_Token::STRUCT))
      {
         ...
         if (tmp->GetParentType() == E_Token::TEMPLATE)
         {
            tmp = tmp->GetPrevType(E_Token::TEMPLATE, prev->GetLevel());
            tmp = tmp->GetPrevNc();
         }
         else
         {
         ...
         {
            if (parent_type == E_Token::CLASS && options::nl_before_class() > tmp->GetNlCount())
            {
               log_rule_B("nl_before_class");
               blank_line_set(tmp, options::nl_before_class);
            }
            else if (parent_type == E_Token::STRUCT && options::nl_before_struct() > tmp->GetNlCount())
            {
         ...
         && prev->GetParentType() == E_Token::NAMESPACE)
      {
         ...
         Chunk *tmp = prev->GetPrevType(E_Token::NAMESPACE, prev->GetLevel());
         tmp = tmp->GetPrevNc();
      ...
      if (  prev->Is(E_Token::BRACE_CLOSE)
         && (  prev->GetParentType() == E_Token::FUNC_DEF
            ...
            || prev->GetParentType() == E_Token::ASSIGN))
      {
      ...
      if (  (  prev->Is(E_Token::SEMICOLON)
            && prev->GetParentType() == E_Token::FUNC_PROTO)
         || is_func_proto_group(prev, E_Token::FUNC_DEF))
      {
         ...
         if (  (options::nl_after_func_proto_group() > pc->GetNlCount())
            && next->IsNotNullChunk()
            && next->GetParentType() != E_Token::FUNC_PROTO
            && !is_func_proto_group(next, E_Token::FUNC_DEF))
         {
      ...
      if (  (  prev->Is(E_Token::SEMICOLON)
            && prev->GetParentType() == E_Token::FUNC_CLASS_PROTO)
         || is_func_proto_group(prev, E_Token::FUNC_CLASS_DEF))
      {
         ...
         if (  (options::nl_after_func_class_proto_group() > pc->GetNlCount())
            && next->IsNot(E_Token::FUNC_CLASS_PROTO)
            && next->GetParentType() != E_Token::FUNC_CLASS_PROTO
            && !is_func_proto_group(next, E_Token::FUNC_CLASS_DEF))
         {
      ...
      if (  (  prev->Is(E_Token::SEMICOLON)
         ...
         && (  prev->GetParentType() == E_Token::STRUCT
            ...
            || prev->GetParentType() == E_Token::CLASS))
      {
         const auto &opt = (prev->GetParentType() == E_Token::CLASS
      ...
      if (  (options::nl_comment_func_def() != 0)
         && pcmt->Is(E_Token::COMMENT_MULTI)
         && pcmt->GetParentType() == E_Token::COMMENT_WHOLE
         && next->IsNotNullChunk()
         && (  next->GetParentType() == E_Token::FUNC_DEF
            || next->GetParentType() == E_Token::FUNC_CLASS_DEF))
      {
         ...
         && next->IsNotNullChunk())
      {
         ...
         if (  prev->Is(E_Token::BRACE_CLOSE)
            && (  prev->GetParentType() == E_Token::CATCH
               || prev->GetParentType() == E_Token::FINALLY))
         {
            if (  next->IsNot(E_Token::BRACE_CLOSE)
               && next->IsNot(E_Token::CATCH)
         ...
         && next->IsNotNullChunk())
      {
         ...
         if (  prev->GetParentType() == E_Token::GETSET
         ...
         && next->IsNotNullChunk())
      {
         ...
         else if (  next->GetParentType() == E_Token::CS_PROPERTY
      ...
      if (  (options::nl_inside_namespace() != 0)
         ...
         && (  (  prev->Is(E_Token::BRACE_OPEN)
               && prev->GetParentType() == E_Token::NAMESPACE)
            || (  next->Is(E_Token::BRACE_CLOSE)
               && next->GetParentType() == E_Token::NAMESPACE)))
      {
      ...
      if (  options::nl_before_whole_file_ifdef() != 0
         ...
         && next->Is(E_Token::PREPROC)
         && next->GetParentType() == E_Token::PP_IF
         ...
         && options::nl_after_whole_file_ifdef() != pc->GetNlCount())
      {
         ...
         if (  pp_start->IsNotNullChunk()
            && pp_start->GetParentType() == E_Token::PP_IF
            && ifdef_over_whole_file()
      ...
      if (  options::nl_before_whole_file_endif() != 0
         ...
         && next->Is(E_Token::PREPROC)
         && next->GetParentType() == E_Token::PP_ENDIF
         && ifdef_over_whole_file()
         ...
         && options::nl_after_whole_file_endif() != pc->GetNlCount())
      {
         ...
         if (  pp_start->IsNotNullChunk()
            && pp_start->GetParentType() == E_Token::PP_ENDIF
            && ifdef_over_whole_file()
...
void newlines_insert_blank_lines()
{
   ...
   for (Chunk *pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnl())
   {
      ...
      else if (  pc->Is(E_Token::FUNC_CLASS_DEF)
              ...
              || pc->Is(E_Token::FUNC_PROTO))
      {
            ...
            && is_class_one_liner(pc))
         {
            ...
            newlines_func_pre_blank_lines(pc, E_Token::FUNC_PROTO);
         }
         else
         {
