   ...
   for (Chunk *pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnl())
   {
      ...
      if (pc->Is(chunk_type))
      {
         ...
         if (mode_local & TP_LEAD)
         {
            ...
            if (  next2->Is(E_Token::PREPROC)
               || (  chunk_type == E_Token::ASSIGN
                  && next2->Is(E_Token::BRACE_OPEN)))
            {
               continue;
            }
            ...
            if (next->GetNlCount() == 1)
            {
               ...
               {
                  // move the E_Token::BOOL to after the newline
                  pc->MoveAfter(next);
               }
            }
         }
