...
void newlines_enum(Chunk *start)
{
   ...
   if (pcClass->Is(E_Token::ENUM_CLASS))
   {
      ...
      if (pcType->Is(E_Token::TYPE))
      {
         ...
         if (pcColon->Is(E_Token::ENUM_COLON))                       // Issue #4040
         {
...
void newlines_enum_entries(Chunk *open_brace, iarf_e av)
{
        ...
        pc = pc->GetNextNc())
   {
      if (  (pc->GetLevel() != (open_brace->GetLevel() + 1))
         || pc->IsNot(E_Token::COMMA)
         || (  pc->Is(E_Token::COMMA)
            && (  pc->GetNext()->GetType() == E_Token::COMMENT_CPP
               || pc->GetNext()->GetType() == E_Token::COMMENT
               || pc->GetNext()->GetType() == E_Token::COMMENT_MULTI)))
      {
         continue;
      }
      newline_iarf(pc, av);
