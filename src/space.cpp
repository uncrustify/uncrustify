...
static iarf_e do_space(Chunk *first, Chunk *second, int &min_sp)
{
   ...
   if (  first->Is(E_Token::COMMENT)                                      // Issue #4327
      && first->GetParentType() == E_Token::COMMENT_EMBED
   ...
   if (  second->Is(E_Token::COMMENT)                                     // Issue #4327
      && second->GetParentType() == E_Token::COMMENT_EMBED
      && (options::sp_emb_cmt_priority()))
   {
   ...
   if (  first->Is(E_Token::VBRACE_OPEN)
      && first->GetPrev()->Is(E_Token::SPAREN_CLOSE)
      && second->IsNot(E_Token::SEMICOLON))
   {
   ...
   if (first->Is(E_Token::SPAREN_CLOSE))
   {
      if (second->Is(E_Token::VBRACE_OPEN))
      {
   ...
   if (  first->Is(E_Token::PP_IGNORE)
      && second->Is(E_Token::PP_IGNORE))
   {
   ...
   if (  first->Is(E_Token::PP)
      || second->Is(E_Token::PP))
   {
   ...
   if (  second->Is(E_Token::POUND)
      && second->TestFlags(PCF_IN_PREPROC)
      && first->GetParentType() != E_Token::MACRO_FUNC)
   {
   ...
   if (first->Is(E_Token::DECLSPEC))  // Issue 1289
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }

   if (  second->Is(E_Token::NEWLINE)
      || second->Is(E_Token::VBRACE_OPEN))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }
   ...
   if (  first->Is(E_Token::VBRACE_OPEN)
      && second->IsNot(E_Token::NL_CONT)
      && second->IsNot(E_Token::SEMICOLON)) // # Issue 1158
   {
      log_rule("FORCE");
      return(IARF_FORCE);
   }

   if (  first->Is(E_Token::VBRACE_CLOSE)
      && second->IsNot(E_Token::NL_CONT))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   ...
   if (second->Is(E_Token::VSEMICOLON))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }

   if (first->Is(E_Token::MACRO_FUNC))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }

   if (second->Is(E_Token::NL_CONT))
   {
   ...
   if (  language_is_set(lang_flag_e::LANG_D)
      && (  first->Is(E_Token::D_ARRAY_COLON)
         || second->Is(E_Token::D_ARRAY_COLON)))
   {
   ...
   if (  first->Is(E_Token::CASE)
      && ((  CharTable::IsKw1(second->GetText()[0])
          || second->Is(E_Token::NUMBER))))
   {
   ...
   if (second->Is(E_Token::FOR_COLON))
   {
      // java
   ...
   if (  first->Is(E_Token::QUESTION)
      && second->Is(E_Token::COND_COLON))
   {
   ...
   if (  first->Is(E_Token::QUESTION)                  // see the tests cpp:34513-34516
      || second->Is(E_Token::QUESTION))
   {
      if (  second->Is(E_Token::QUESTION)
   ...
   if (  first->Is(E_Token::COND_COLON)
      || second->Is(E_Token::COND_COLON))
   {
      if (  second->Is(E_Token::COND_COLON)
      ...
      // Issue #2596
      // replace "if (first->Is(E_Token::WORD) && second->Is(E_Token::COND_COLON))"
   ...
   if (  language_is_set(lang_flag_e::LANG_D)
      && (  first->Is(E_Token::RANGE)
         || second->Is(E_Token::RANGE)))
   {
   ...
   if (  first->Is(E_Token::COLON)
      && first->GetParentType() == E_Token::SQL_EXEC)
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }
   ...
   if (  first->Is(E_Token::FPAREN_CLOSE)
      && first->GetParentType() == E_Token::MACRO_FUNC)
   {
   ...
   if (second->Is(E_Token::SEMICOLON))                       // see the tests cpp:34517-34519
   {
      if (  first->Is(E_Token::VBRACE_OPEN)                  // Issue #2942
         && first->GetPrev()->Is(E_Token::SPAREN_CLOSE))
      {
         if (  first->GetParentType() == E_Token::IF
            || first->GetParentType() == E_Token::FOR
            || first->GetParentType() == E_Token::WHILE)
         {
      ...
      // Issue #4094-03
      if (second->GetParentType() == E_Token::FOR)
      {
         if (first->Is(E_Token::SPAREN_OPEN))
         {
            // empty, e.g. for (;;)
            //                 ^ is first
            //                  ^ is second
   ...
   if (  (  second->Is(E_Token::COMMENT)
         || second->Is(E_Token::COMMENT_CPP))
      && (  first->Is(E_Token::PP_ELSE)
         || first->Is(E_Token::PP_ENDIF)))
   {
      if (second->Is(E_Token::COMMENT_CPP))
      {
         second->SetType(E_Token::COMMENT_CPP_ENDIF);
      }
      else
      {
         second->SetType(E_Token::COMMENT_ENDIF);
      }
   ...
   if (  options::sp_before_tr_cmt() != IARF_IGNORE
      && second->GetParentType() == E_Token::COMMENT_END)
   {
   ...
   if (second->GetParentType() == E_Token::COMMENT_END)
   {
   ...
   if (first->Is(E_Token::SEMICOLON))                        // see the tests cpp:34517-34519
   {
      if (first->GetParentType() == E_Token::FOR)
      {
         if (second->Is(E_Token::SPAREN_CLOSE))
         {
         ...
         if (second->IsNot(E_Token::SPAREN_CLOSE))  // Issue 1324
         {
            ...
            return(options::sp_after_semi_for());
              ...
              && second->IsNot(E_Token::BRACE_CLOSE)) // issue #197
      {
   ...
   if (  (  first->Is(E_Token::NEG)
         || first->Is(E_Token::POS)
         || first->Is(E_Token::ARITH)
         || first->Is(E_Token::SHIFT))
      && (  second->Is(E_Token::NEG)
         || second->Is(E_Token::POS)
         || second->Is(E_Token::ARITH)
         || second->Is(E_Token::SHIFT)))
   {
      log_rule("ADD");
      return(IARF_ADD);
   }
   ...
   if (first->Is(E_Token::RETURN))
   {
      if (  second->Is(E_Token::PAREN_OPEN)
         && second->GetParentType() == E_Token::RETURN)
      {
         // Add or remove space between 'return' and '('.
         log_rule("sp_return_paren");
         return(options::sp_return_paren());
      }
      else if (  second->Is(E_Token::BRACE_OPEN)
              && second->GetParentType() == E_Token::BRACED_INIT_LIST)
      {
         // Add or remove space between 'return' and '{'.
   ...
   if (  (  first->Is(E_Token::FPAREN_OPEN)
         || first->Is(E_Token::SPAREN_OPEN)
         || first->Is(E_Token::PAREN_OPEN))
      && second->Is(E_Token::DEREF))               // Issue #4459
   {
   ...
   if (  first->Is(E_Token::PAREN_CLOSE)
      && second->Is(E_Token::DC_MEMBER)
      && second->GetNext()->GetType() == E_Token::FUNC_CALL)
   {
      log_rule("sp_after_cast");
      return(options::sp_after_cast());
   }

   if (second->Is(E_Token::DC_MEMBER))
   {
      ...
      case E_Token::NEW:
      case E_Token::SARITH:
      case E_Token::SCOMPARE:
      case E_Token::OPERATOR:
      case E_Token::ACCESS:
      case E_Token::QUALIFIER:
      case E_Token::RETURN:
      case E_Token::SIZEOF:
      case E_Token::DECLTYPE:
      case E_Token::STRUCT:
      case E_Token::THROW:
      case E_Token::TYPEDEF:
      case E_Token::TYPENAME:
      case E_Token::UNION:
      case E_Token::USING:
         log_rule("FORCE");
         return(IARF_FORCE);
      ...
      if (  (  first->Is(E_Token::WORD)
            || first->Is(E_Token::TYPE)
            || first->Is(E_Token::PAREN_CLOSE)
   ...
   // see the tests c-sharp:12200-12202
   {
      ...
      if (second->Is(E_Token::ANGLE_CLOSE))
      {
         log_rule("IGNORE");
         return(IARF_IGNORE);
      }
   ...
   {
      if (  first->Is(E_Token::FPAREN_CLOSE)
         && (  second->Is(E_Token::FPAREN_CLOSE)
            || second->Is(E_Token::COMMA)))
      {
   ...
   if (second->Is(E_Token::COMMA))
   {
      ...
      if (  first->Is(E_Token::PAREN_OPEN)
         || first->Is(E_Token::FPAREN_OPEN))
      {
   ...
   if (second->Is(E_Token::ELLIPSIS))
   {
      ...
      if (first->Is(E_Token::TAG_COLON))
      {
         log_rule("FORCE");
         return(IARF_FORCE);
      }

      if (first->Is(E_Token::BYREF))                         // Issue #3309
      {
         log_rule("sp_byref_ellipsis");
         return(options::sp_byref_ellipsis());
      }

      if (first->Is(E_Token::PARAMETER_PACK))                // Issue #3309
      {
   ...
   if (first->Is(E_Token::ELLIPSIS))
   {
      if (second->Is(E_Token::PARAMETER_PACK))                // Issue #3309
      {
      ...
      if (  second->Is(E_Token::PAREN_OPEN)
         && first->GetPrev()->Is(E_Token::SIZEOF))
      {
         // Add or remove space between 'sizeof...' and '('.
   ...
   if (second->Is(E_Token::TAG_COLON))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }
   ...
   // handle '~'
   if (first->Is(E_Token::DESTRUCTOR))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }
   ...
   if (  language_is_set(lang_flag_e::LANG_OC)
      && first->Is(E_Token::CATCH)
      && second->Is(E_Token::SPAREN_OPEN)
   ...
   if (  language_is_set(lang_flag_e::LANG_OC)
      && (  first->Is(E_Token::PAREN_CLOSE)
         || first->Is(E_Token::OC_CLASS)
         || first->Is(E_Token::WORD))
      && second->Is(E_Token::ANGLE_OPEN)
      && (  second->GetParentType() == E_Token::OC_PROTO_LIST
         || second->GetParentType() == E_Token::OC_GENERIC_SPEC)
   ...
   // "if (" vs. "if("
   if (second->Is(E_Token::SPAREN_OPEN))
   {
      // Add or remove space after 'do' between 'while' and '('. Issue #995
      if (  first->Is(E_Token::WHILE_OF_DO)
   ...
   if (  first->Is(E_Token::LAMBDA)
      || second->Is(E_Token::LAMBDA))
   {
   ...
   if (  (options::sp_cpp_lambda_assign() != IARF_IGNORE)
      && (  (  first->Is(E_Token::SQUARE_OPEN)
            && first->GetParentType() == E_Token::CPP_LAMBDA
            && second->Is(E_Token::ASSIGN))
         || (  first->Is(E_Token::ASSIGN)
            && second->Is(E_Token::SQUARE_CLOSE)
            && second->GetParentType() == E_Token::CPP_LAMBDA)))
   {
      log_rule("sp_cpp_lambda_assign");
      return(options::sp_cpp_lambda_assign());
   }

   if (  first->Is(E_Token::SQUARE_CLOSE)
      && first->GetParentType() == E_Token::CPP_LAMBDA)
   {
      // Handle the special lambda case for C++11:
      //    [](Something arg){.....}
      ...
      if (second->Is(E_Token::LPAREN_OPEN))
      {
   ...
   if (first->Is(E_Token::LPAREN_OPEN))
   {
      if (second->Is(E_Token::LPAREN_CLOSE))
      {
   ...
   if (first->Is(E_Token::LPAREN_CLOSE))
   {
      if (second->Is(E_Token::BRACE_OPEN))
      {
   ...
   if (  first->Is(E_Token::BRACE_CLOSE)
      && first->GetParentType() == E_Token::CPP_LAMBDA
      && second->Is(E_Token::FPAREN_OPEN))
   {
   ...
   if (first->Is(E_Token::ENUM))
   {
      ...
      else if (second->Is(E_Token::FPAREN_OPEN))
      {
   ...
   if (second->Is(E_Token::ASSIGN))
   {
      ...
      if (  (options::sp_assign_default() != IARF_IGNORE)
         && second->GetParentType() == E_Token::FUNC_PROTO)
      {
   ...
   if (second->Is(E_Token::ASSIGN_DEFAULT_ARG))
   {
      // Add or remove space around assignment operator '=' in a prototype.
      // If set to ignore, use sp_assign.
      if (  (options::sp_assign_default() != IARF_IGNORE)
   ...
   if (first->Is(E_Token::ASSIGN))
   {
      ...
      // If set to ignore, use sp_assign.
      if (  (options::sp_assign_default() != IARF_IGNORE)
         && first->GetParentType() == E_Token::FUNC_PROTO)
      {
         log_rule("sp_assign_default");
         return(options::sp_assign_default());
      }
   ...
   if (  first->Is(E_Token::TRAILING_RET)
      || first->Is(E_Token::CPP_LAMBDA_RET)
      || second->Is(E_Token::TRAILING_RET)
      || second->Is(E_Token::CPP_LAMBDA_RET))
   {
   ...
   if (first->Is(E_Token::ASSIGN_DEFAULT_ARG))
   {
      // Add or remove space around assignment operator '=' in a prototype.
      // If set to ignore, use sp_assign.
      if (  (options::sp_assign_default() != IARF_IGNORE)
   ...
   if (first->Is(E_Token::ENUM_COLON))                      // Issue #4040
   {
      // Add or remove space around assignment ':' in enum.
   ...
   if (second->Is(E_Token::ENUM_COLON))                    // Issue #4040
   {
      // Add or remove space around assignment ':' in enum.
      log_rule("sp_enum_colon");
      return(options::sp_enum_colon());
   ...
   if (first->Is(E_Token::BIT_COLON))                      // Issue #4040
   {
   ...
   if (second->Is(E_Token::BIT_COLON))                    // Issue #4040
   {
   ...
   if (  first->Is(E_Token::OC_AVAILABLE_VALUE)
      || second->Is(E_Token::OC_AVAILABLE_VALUE))
   {
      log_rule("IGNORE");
      return(IARF_IGNORE);
   }
   ...
   if (language_is_set(lang_flag_e::LANG_OC))
   {
      if (second->Is(E_Token::OC_BLOCK_CARET))
      {
      ...
      if (first->Is(E_Token::OC_BLOCK_CARET))
      {
      ...
      if (second->Is(E_Token::OC_MSG_FUNC))
      {
         if (  (options::sp_after_oc_msg_receiver() == IARF_REMOVE)
            && (  first->IsNot(E_Token::SQUARE_CLOSE)
               && first->IsNot(E_Token::FPAREN_CLOSE)
               && first->IsNot(E_Token::PAREN_CLOSE)))
         {
            log_rule("FORCE");
            return(IARF_FORCE);
         }
   ...
   if (  language_is_set(lang_flag_e::LANG_CPP)
      && first->Is(E_Token::BYREF)
      && second->Is(E_Token::SQUARE_OPEN)
      && second->GetParentType() != E_Token::OC_MSG
      && second->GetParentType() != E_Token::CS_SQ_STMT)
   {
   ...
   // "a [x]" vs. "a[x]"
   if (  second->Is(E_Token::SQUARE_OPEN)
      && (  second->GetParentType() != E_Token::OC_MSG
         && second->GetParentType() != E_Token::CS_SQ_STMT
         && second->GetParentType() != E_Token::CPP_LAMBDA))
   {
   ...
   if (  (options::sp_angle_shift() != IARF_IGNORE)
      && first->Is(E_Token::ANGLE_CLOSE)
      && second->Is(E_Token::ANGLE_CLOSE))
   {
   ...
   if (  first->Is(E_Token::ANGLE_OPEN)
      || second->Is(E_Token::ANGLE_CLOSE))
   {
      if (  first->Is(E_Token::ANGLE_OPEN)
         && second->Is(E_Token::ANGLE_CLOSE))
   ...
   if (second->Is(E_Token::ANGLE_OPEN))
   {
      if (  first->Is(E_Token::TEMPLATE)
      ...
      if (first->IsNot(E_Token::QUALIFIER))
      {
         // Add or remove space before '<'.
   ...
   if (first->Is(E_Token::ANGLE_CLOSE))
   {
      if (  second->Is(E_Token::WORD)
         || CharTable::IsKw1(second->GetText()[0]))
      {
      ...
      // Issue #4094-02
      if (  second->Is(E_Token::FPAREN_OPEN)
         || second->Is(E_Token::PAREN_OPEN))
      {
      ...
      if (  second->IsNot(E_Token::BYREF)
         && second->IsNot(E_Token::PTR_TYPE)
         && second->IsNot(E_Token::BRACE_OPEN)
         && second->IsNot(E_Token::PAREN_CLOSE))
      {
         if (  second->Is(E_Token::CLASS_COLON)
         ...
         // by default.
         if (  second->Is(E_Token::FPAREN_CLOSE)
   ...
   if (first->Is(E_Token::BYREF))                             // see the tests cpp:34509-34512
   {
      if (second->Is(E_Token::PAREN_OPEN))
      {
      ...
      else if (  first->GetParentType() == E_Token::FUNC_DEF        // Issue #3197, #3210
              || first->GetParentType() == E_Token::FUNC_PROTO)
      {
         ...
         return(options::sp_after_byref_func());
              ...
              && (  options::sp_after_byref() != IARF_IGNORE
                 || (  !second->Is(E_Token::FUNC_PROTO)
                    && !second->Is(E_Token::FUNC_DEF))))
      {
   ...
   if (  second->Is(E_Token::BYREF)
      && !first->Is(E_Token::PAREN_OPEN))              // Issue #1804
   {
      if (  second->GetParentType() == E_Token::FUNC_DEF     // Issue #3197, #3210
         || second->GetParentType() == E_Token::FUNC_PROTO)
      {
      ...
      if (  next->IsNotNullChunk()
         && (  next->Is(E_Token::COMMA)
            || next->Is(E_Token::PAREN_CLOSE)                            // Issue #3691
            || next->Is(E_Token::FPAREN_CLOSE)
            || next->Is(E_Token::SEMICOLON)
            || next->Is(E_Token::ANGLE_CLOSE)))                          // Issue #4064
      {
   ...
   if (first->Is(E_Token::SPAREN_CLOSE))
   {
      if (second->Is(E_Token::BRACE_OPEN))
      {
         if (second->GetParentType() == E_Token::CATCH)
         {
            if (  language_is_set(lang_flag_e::LANG_OC)
   ...
   // Issue #4094-07
   if (  second->Is(E_Token::FPAREN_OPEN)
      && first->GetParentType() == E_Token::OPERATOR
   ...
   if (  first->Is(E_Token::MACRO_OPEN)
      || first->Is(E_Token::MACRO_CLOSE)
      || first->Is(E_Token::MACRO_ELSE)
      || first->Is(E_Token::MACRO_NO_INDENT))
   {
      if (second->Is(E_Token::FPAREN_OPEN))
      {
   ...
   if (  first->Is(E_Token::FUNC_CALL)
      || first->Is(E_Token::FUNCTION)                        // Issue #2665
      || first->Is(E_Token::FUNC_CTOR_VAR)
      || first->Is(E_Token::CNG_HASINC)
      || first->Is(E_Token::CNG_HASINCN)
      || (  first->Is(E_Token::BRACE_CLOSE)
         && first->GetParentType() == E_Token::BRACED_INIT_LIST
         && second->Is(E_Token::FPAREN_OPEN))
      || (  first->Is(E_Token::FUNC_VAR)                     // Issue #3852
         && second->Is(E_Token::PAREN_OPEN)))
   {
   ...
   if (first->Is(E_Token::FUNC_CALL_USER))
   {
   ...
   if (  first->Is(E_Token::CPP_CAST)
      || first->Is(E_Token::TYPE_WRAP))
   {
         ...
         || second->Is(E_Token::FPAREN_OPEN)))
   {
      ...
      if (  first->GetParentType() == E_Token::C_CAST
         || first->GetParentType() == E_Token::D_CAST)
      {
   ...
   if (first->Is(E_Token::TPAREN_CLOSE))
   {
   ...
   // Issue #4094-09
   if (  first->Is(E_Token::FUNC_PROTO)
      || (  second->Is(E_Token::FPAREN_OPEN)
         && second->GetParentType() == E_Token::FUNC_PROTO))
   {
   ...
   // Issue #4094-12
   if (  first->Is(E_Token::FUNC_CLASS_DEF)
      || first->Is(E_Token::FUNC_CLASS_PROTO))
   {
   ...
   // Issue #4094-08
   if (  first->Is(E_Token::BRACE_OPEN)
      && second->Is(E_Token::BRACE_CLOSE))
   {
      // Add or remove space inside '{}'.
      // if empty.
   ...
   if (  (  first->Is(E_Token::TYPE)                           // Issue #2428
         || first->Is(E_Token::ANGLE_CLOSE))
      && second->Is(E_Token::BRACE_OPEN)
      && second->GetParentType() == E_Token::BRACED_INIT_LIST)
   {
      ...
      if (  arg != IARF_IGNORE
         || first->GetParentType() != E_Token::DECLTYPE)
      {
         // 'int{9}' vs. 'int {9}'
   ...
   if (  (  first->Is(E_Token::WORD)                           // Issue #2428
         || first->Is(E_Token::SQUARE_CLOSE)
         || first->Is(E_Token::TSQUARE))
      && second->Is(E_Token::BRACE_OPEN)
      && second->GetParentType() == E_Token::BRACED_INIT_LIST)
   {
   ...
   if (second->Is(E_Token::BRACE_CLOSE))
   {
      if (second->GetParentType() == E_Token::ENUM)
      {
      ...
      if (  second->GetParentType() == E_Token::STRUCT
         || second->GetParentType() == E_Token::UNION)
      {
         ...
         return(options::sp_inside_braces_struct());
              ...
              && second->GetParentType() == E_Token::OC_AT
      ...
      if (second->GetParentType() == E_Token::BRACED_INIT_LIST)
      {
   ...
   if (first->Is(E_Token::D_CAST))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }

   if (  first->Is(E_Token::PP_DEFINED)
      && second->Is(E_Token::PAREN_OPEN))
   {
   ...
   if (  first->Is(E_Token::MEMBER)
      || second->Is(E_Token::MEMBER))
   {
   ...
   if (first->Is(E_Token::C99_MEMBER))
   {
      ...
      && second->Is(E_Token::BRACE_OPEN))
   {
      if (  language_is_set(lang_flag_e::LANG_JAVA)
         && second->GetParentType() == E_Token::DOUBLE_BRACE)
      {
   ...
   if (  first->Is(E_Token::D_TEMPLATE)
      || second->Is(E_Token::D_TEMPLATE))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }

   if (  first->Is(E_Token::ELSE)
      && second->Is(E_Token::BRACE_OPEN))
   {
   ...
   if (  first->Is(E_Token::ELSE)
      && second->Is(E_Token::ELSEIF))
   {
      log_rule("FORCE");
      return(IARF_FORCE);
   }

   if (  first->Is(E_Token::FINALLY)
      && second->Is(E_Token::BRACE_OPEN))
   {
   ...
   if (  first->Is(E_Token::WORD)
      && second->Is(E_Token::BRACE_OPEN))
   {
      if (first->GetParentType() == E_Token::NAMESPACE)
      {
   ...
   if (  language_is_set(lang_flag_e::LANG_D)
      && second->Is(E_Token::PAREN_OPEN)
      && second->GetParentType() == E_Token::INVARIANT)
   {
   ...
   {
      if (  language_is_set(lang_flag_e::LANG_D)
         && first->GetParentType() == E_Token::INVARIANT)
      {
      ...
      if (first->GetParentType() == E_Token::DELEGATE)
      {
         log_rule("ADD");
         return(IARF_ADD);
      }
      ...
      if (first->GetParentType() == E_Token::STATE)
      {
         log_rule("ADD");
         return(IARF_ADD);
      }
      ...
      if (first->GetParentType() == E_Token::NEW)
      {
   ...
   if (  (  first->IsString("(")
      ...
      || (  first->IsString(")")
         && second->IsString(")")))
   {
      if (second->GetParentType() == E_Token::FUNC_CALL_USER)
      {
      ...
      if (  options::sp_sparen_paren() != IARF_IGNORE
         && (  first->Is(E_Token::SPAREN_OPEN)
            || second->Is(E_Token::SPAREN_CLOSE)))
      {
   ...
   if (  first->Is(E_Token::FPAREN_OPEN)
      || second->Is(E_Token::FPAREN_CLOSE))
   {
      if (  (first->GetParentType() == E_Token::FUNC_CALL_USER)
         || (  (second->GetParentType() == E_Token::FUNC_CALL_USER)
            && (  (first->Is(E_Token::WORD))
               || (first->Is(E_Token::SQUARE_CLOSE)))))
      {
   ...
   // functor "foo(...)" vs. "foo( ... )"
   if (  first->Is(E_Token::RPAREN_OPEN)
      || second->Is(E_Token::RPAREN_CLOSE))                  // Issue #3914
   {
      if (  (first->GetParentType() == E_Token::FUNC_CALL_USER)
         || (  (second->GetParentType() == E_Token::FUNC_CALL_USER)
            && (  (first->Is(E_Token::WORD))
   ...
   // "foo(...)" vs. "foo( ... )"
   if (  first->Is(E_Token::TPAREN_OPEN)
      || second->Is(E_Token::TPAREN_CLOSE))
   {
   ...
   if (  language_is_set(lang_flag_e::LANG_OC)
      && first->Is(E_Token::PAREN_CLOSE))
   {
      if (  first->TestFlags(PCF_OC_RTYPE) // == E_Token::OC_RTYPE)
         && (  first->GetParentType() == E_Token::OC_MSG_DECL
            || first->GetParentType() == E_Token::OC_MSG_SPEC))
      {
      ...
      if (  first->GetParentType() == E_Token::OC_MSG_SPEC
         || first->GetParentType() == E_Token::OC_MSG_DECL)
      {
      ...
      if (  first->GetParentType() == E_Token::OC_SEL
         && second->IsNot(E_Token::SQUARE_CLOSE))
      {
   ...
   {
      if (  (  first->Is(E_Token::PAREN_OPEN)
            && (  first->GetParentType() == E_Token::OC_SEL
               || first->GetParentType() == E_Token::OC_PROTOCOL))
         || (  second->Is(E_Token::PAREN_CLOSE)
            && (  second->GetParentType() == E_Token::OC_SEL
               || second->GetParentType() == E_Token::OC_PROTOCOL)))
      {
   ...
   if (  second->Is(E_Token::PAREN_OPEN)
      && (  first->Is(E_Token::OC_SEL)
         || first->Is(E_Token::OC_PROTOCOL)))
   {
   ...
   {
      if (  first->GetParentType() == E_Token::C_CAST
         || first->GetParentType() == E_Token::CPP_CAST
         || first->GetParentType() == E_Token::D_CAST)
      {
   ...
   if (second->Is(E_Token::PAREN_CLOSE))
   {
      if (  second->GetParentType() == E_Token::C_CAST
         || second->GetParentType() == E_Token::CPP_CAST
         || second->GetParentType() == E_Token::D_CAST)
      {
         // Add or remove spaces inside cast parentheses.
         log_rule("sp_inside_paren_cast");
         return(options::sp_inside_paren_cast());
      ...
      if (second->GetParentType() == E_Token::NEW)
      {
   ...
   // Issue #4094-06
   if (  first->Is(E_Token::SQUARE_OPEN)
      && second->Is(E_Token::SQUARE_CLOSE))
   {
      // Add or remove space inside '[]'.
      // if empty.
   ...
   {
      if (  language_is_set(lang_flag_e::LANG_OC)
         && (  (  first->GetParentType() == E_Token::OC_AT
               && first->Is(E_Token::SQUARE_OPEN))
            || (  second->GetParentType() == E_Token::OC_AT
               && second->Is(E_Token::SQUARE_CLOSE)))
   ...
   if (first->Is(E_Token::CLASS_COLON))
   {
      if (  first->GetParentType() == E_Token::OC_CLASS
         && (  first->GetPrevType(E_Token::OC_INTF, first->GetLevel(), E_Scope::ALL)->IsNullChunk()
            && first->GetPrevType(E_Token::OC_IMPL, first->GetLevel(), E_Scope::ALL)->IsNullChunk()))
      {
   ...
   {
      if (  language_is_set(lang_flag_e::LANG_OC)
         && second->GetParentType() == E_Token::OC_CLASS
         && (  second->GetPrevType(E_Token::OC_INTF, second->GetLevel(), E_Scope::ALL)->IsNullChunk()
            && second->GetPrevType(E_Token::OC_IMPL, second->GetLevel(), E_Scope::ALL)->IsNullChunk()))
      {
         if (/* second->GetParentType() == E_Token::OC_CLASS && */   // Already true
            second->GetPrevType(E_Token::OC_INTF, second->GetLevel(), E_Scope::ALL)->IsNullChunk())
         {
   ...
   if (first->Is(E_Token::CONSTR_COLON))
   {
   ...
   if (second->Is(E_Token::CONSTR_COLON))
   {
   ...
   if (second->Is(E_Token::CASE_COLON))
   {
   ...
   if (first->Is(E_Token::DOT))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }

   if (second->Is(E_Token::DOT))
   {
      log_rule("ADD");
      return(IARF_ADD);
   }

   if (  first->Is(E_Token::NULLCOND)
      || second->Is(E_Token::NULLCOND))
   {
      // Add or remove space around the '.' or '->' operators.
   ...
   if (  first->Is(E_Token::ARITH)
      || first->Is(E_Token::SHIFT)
      || first->Is(E_Token::CARET)
      || second->Is(E_Token::ARITH)
      || second->Is(E_Token::SHIFT)
      || second->Is(E_Token::CARET))
   {
      // Add or remove space around arithmetic operators '+' and '-'.
      // Overrides sp_arith.
      if (options::sp_arith_additive() != IARF_IGNORE)
      {
         auto arith_char = (  first->Is(E_Token::ARITH)
                           || first->Is(E_Token::SHIFT)
                           || first->Is(E_Token::CARET))
   ...
   if (  first->Is(E_Token::BOOL)
      || second->Is(E_Token::BOOL))
   {
   ...
   if (  first->Is(E_Token::COMPARE)
      || second->Is(E_Token::COMPARE))
   {
   ...
   if (first->Is(E_Token::PTR_TYPE))                            // see the tests cpp:34505-34508
   {
      if (  second->Is(E_Token::FPAREN_OPEN)
         || second->Is(E_Token::TPAREN_OPEN))
      {
      ...
      else if (second->Is(E_Token::PTR_TYPE))
      {
      ...
      else if (second->Is(E_Token::FUNC_VAR))
      {
      ...
      else if (second->Is(E_Token::FUNC_TYPE))
      {
         // Add or remove space between the pointer star '*' and the name of the
      ...
      else if (  first->GetParentType() == E_Token::FUNC_DEF
              || first->GetParentType() == E_Token::FUNC_PROTO
              || first->GetParentType() == E_Token::FUNC_VAR)
      {
      ...
      else if (CharTable::IsKw1(second->GetText()[0]))
      {
         ...
         else if (/* first->GetParentType() == E_Token::FUNC_VAR || */    // Always false
            first->GetParentType() == E_Token::FUNC_TYPE)
         {
            // Add or remove space after pointer caret '^', if followed by a word.
            log_rule("sp_after_ptr_block_caret");
            return(options::sp_after_ptr_block_caret());
         }
         else if (second->Is(E_Token::QUALIFIER))
         {
      ...
      else if (second->Is(E_Token::PAREN_OPEN))
      {
         // Add or remove space after pointer star '*', if followed by a word.
         log_rule("sp_after_ptr_star");                               // ptr_star 1
         return(options::sp_after_ptr_star());
   ...
   if (  second->Is(E_Token::PTR_TYPE)
      && first->IsNot(E_Token::IN))
   {
      // look back for '->' type is TRAILING_RET
      if (token_is_within_trailing_return(second))
      {
         if (first->Is(E_Token::QUALIFIER))
         {
      ...
      {
         next = next->GetNext();
      } while (next->Is(E_Token::PTR_TYPE));

      if (  next->Is(E_Token::FUNC_DEF)
         || next->Is(E_Token::FUNC_PROTO))
      {
      ...
      else
      {
         ...
         while (  next->Is(E_Token::PTR_TYPE)
               || next->Is(E_Token::BYREF))   // Issue #4438
         {
            next = next->GetNextNc();
         }

         if (  next->IsNotNullChunk()
            && next->IsNot(E_Token::WORD))
         {
            if (first->Is(E_Token::QUALIFIER))
            {
            ...
            else
            {
               ...
               else if (next->Is(E_Token::OPERATOR))
               {
               ...
               else
               {
                  ...
                  if (next_next->IsNotNullChunk() && next_next->Is(E_Token::DC_MEMBER))
                  {
   ...
   if (  second->Is(E_Token::FUNC_PROTO)
      || second->Is(E_Token::FUNC_DEF))
   {
      if (  first->IsNot(E_Token::PTR_TYPE)
         && first->IsNot(E_Token::BYREF))
      {
   ...
   if (first->Is(E_Token::BRACE_OPEN))
   {
      if (first->GetParentType() == E_Token::ENUM)
      {
         // Add or remove space inside enum '{' and '}'.
         log_rule("sp_inside_braces_enum");
         return(options::sp_inside_braces_enum());
      ...
      if (  first->GetParentType() == E_Token::STRUCT
         || first->GetParentType() == E_Token::UNION)
      {
         // Fix for issue #1240  adding space in struct initializers
      ...
      else if (  first->GetParentType() == E_Token::OC_AT
              && options::sp_inside_braces_oc_dict() != IARF_IGNORE)
      {
         // (OC) Add or remove space inside Objective-C boxed dictionary '{' and '}'
         log_rule("sp_inside_braces_oc_dict");
      ...
      if (first->GetParentType() == E_Token::BRACED_INIT_LIST)
      {
         // Add or remove space between nested braces, i.e. '{{' vs. '{ {'.
         // only to help the vim command }}}}
         if (  options::sp_brace_brace() != IARF_IGNORE
   ...
   if (  first->Is(E_Token::BRACE_CLOSE)
      ...
      && (  first->GetParentType() == E_Token::ENUM
         || first->GetParentType() == E_Token::STRUCT
         || first->GetParentType() == E_Token::UNION))
   {
   ...
   if (  language_is_set(lang_flag_e::LANG_D)
      && second->Is(E_Token::PAREN_OPEN)
      && second->GetParentType() == E_Token::TEMPLATE)
   {
   ...
   if (  first->Is(E_Token::PAREN_CLOSE)
      && first->GetParentType() == E_Token::DECLTYPE
      && (  second->Is(E_Token::WORD)
         || second->Is(E_Token::BRACE_OPEN)
         || second->Is(E_Token::FUNC_CALL)))
   {
   ...
   if (  language_is_set(lang_flag_e::LANG_D)
      && first->Is(E_Token::PAREN_CLOSE)
      && first->GetParentType() == E_Token::D_TEMPLATE)
   {
      if (second->GetParentType() == E_Token::USING_ALIAS)
      {
   ...
   if (  first->Is(E_Token::TYPE)                   // Issue #3457
      && second->Is(E_Token::COLON))
   {
   ...
   if (first->Is(E_Token::NOT))
   {
      if (  second->Is(E_Token::NOT)
   ...
   if (  first->Is(E_Token::POS)
      || first->Is(E_Token::NEG))
   {
   ...
   if (  first->Is(E_Token::INCDEC_BEFORE)
      || second->Is(E_Token::INCDEC_AFTER))
   {
   ...
   if (second->Is(E_Token::CS_SQ_COLON))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }

   if (first->Is(E_Token::CS_SQ_COLON))
   {
      log_rule("FORCE");
      return(IARF_FORCE);
   }
   ...
   if (  language_is_set(lang_flag_e::LANG_OC)
      && first->Is(E_Token::OC_DICT_COLON))
   {
   ...
   if (  language_is_set(lang_flag_e::LANG_OC)
      && second->Is(E_Token::OC_DICT_COLON))
   {
   ...
   if (  language_is_set(lang_flag_e::LANG_OC)
      && second->Is(E_Token::OC_COLON))
   {
      if (  first->TestFlags(PCF_IN_OC_MSG)
         && (  first->Is(E_Token::OC_MSG_FUNC)
            || first->Is(E_Token::OC_MSG_NAME)))
      {
         // (OC) Add or remove space before the colon in message specs,
         // i.e. '[object setValue:1];' vs. '[object setValue :1];'.
   ...
   if (  first->Is(E_Token::NEW)
      || first->Is(E_Token::DELETE)
      || (  first->Is(E_Token::TSQUARE)
         && first->GetParentType() == E_Token::DELETE))
   {
   ...
   if (  second->Is(E_Token::TYPE)
      && (  (  first->Is(E_Token::STRING)
            && first->GetParentType() == E_Token::EXTERN)
         || (  first->Is(E_Token::FPAREN_CLOSE)
            && first->GetParentType() == E_Token::ATTRIBUTE)))
   {
      log_rule("FORCE");
      return(IARF_FORCE);  /* TODO: make this configurable? */
   }

   if (  first->Is(E_Token::STRING)                        // Issue #4176
      && second->Is(E_Token::STRING))
   {
      log_rule("sp_string_string");
      return(options::sp_string_string());
   }

   if (first->Is(E_Token::NOEXCEPT))
   {
   ...
   // Issue #2098
   if (  first->Is(E_Token::PP_PRAGMA)
      && second->Is(E_Token::PREPROC_BODY))
   {
      log_rule("REMOVE");
      return(IARF_REMOVE);
   }
   ...
   // Issue #1733
   if (  first->Is(E_Token::OPERATOR_VAL)
      && second->Is(E_Token::TYPE))
   {
      log_rule("IGNORE");
      return(IARF_IGNORE);
   }
   ...
   {
      if (  (  it.first == E_Token::UNKNOWN
            || it.first == first->GetType())
         && (  it.second == E_Token::UNKNOWN
   ...
   // Issue #2386
   if (  first->Is(E_Token::FORM_FEED)
      || second->Is(E_Token::FORM_FEED))
   {
      log_rule("IGNORE");
      return(IARF_IGNORE);
   }
   ...
   if (  first->Is(E_Token::MACRO_FUNC_CALL)
      && second->Is(E_Token::FPAREN_OPEN))
   {
      log_rule("IGNORE");
      return(IARF_IGNORE);
   ...
   if (  first->Is(E_Token::CASE_ELLIPSIS)
      && second->Is(E_Token::NUMBER))
   {
   ...
   if (  first->Is(E_Token::NUMBER)
      && second->Is(E_Token::CASE_ELLIPSIS))
   {
      // Add or remove space before the variadic '...' when preceded by a
      // non-punctuator.
      // The value REMOVE will be overridden with FORCE
   ...
   LOG_FMT(LSPACE, "   { E_Token::%s,    E_Token::%s},\n",
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      else
      {
         ...
         case IARF_ADD:
         {
            ...
            // Handle the condition of a E_Token::VBRACE_OPEN with more complicated spacing mechanics.
            ...
            && column < next->GetOrigCol())
         {
            ...
            // Add or remove space between #else or #endif and a trailing comment.
            if (  (  options::sp_before_tr_cmt() == IARF_IGNORE
                  || next->GetParentType() != E_Token::COMMENT_END)
               && (  options::sp_endif_cmt() == IARF_IGNORE
                  || (  pc->IsNot(E_Token::PP_ELSE)
                     && pc->IsNot(E_Token::PP_ENDIF))))
            {
