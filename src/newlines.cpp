...
Chunk *newline_add_between(Chunk *start, Chunk *end)
{
    ...
    */
   for (Chunk *pc = start; pc != end; pc = pc->get_next())
   {
      if (chunk_is_newline(pc))
      {
         return(pc);
   ...
   if (chunk_is_token(end, CT_BRACE_OPEN))
   {
      ...
      if (chunk_is_comment(pc))
      {
         ...
         if (chunk_is_newline(pc))
         {
            ...
            Chunk *pc1 = chunk_get_next_nc_nnl(end);

            if (!chunk_is_newline(pc1))
            {
               // yes, go back
               Chunk *pc2 = pc1->get_prev();
               pc = pc2;
            }
         }
...
void newlines_sparens()
{
           ...
           sparen_open, CT_SPAREN_OPEN, ANY_LEVEL))
   {
      ...
      Chunk *sparen_content_start = sparen_open->get_next_nnl();
      Chunk *sparen_content_end   = sparen_close->get_prev_nnl();
      bool  is_multiline          = (
...
static bool newlines_if_for_while_switch(Chunk *start, iarf_e nl_opt)
{
   ...
   if (chunk_is_token(pc, CT_SPAREN_OPEN))
   {
      ...
      Chunk *brace_open  = chunk_get_next_nc_nnl(close_paren);
         ...
         && one_liner_nl_ok(brace_open))
      {
         ...
         {
            while ((pc = pc->get_next()) != close_paren)
            {
               if (chunk_is_newline(pc))
               {
                  nl_opt = IARF_ADD;
         ...
         else
         {
            ...
            Chunk *next = chunk_get_next_nc_nnl(brace_open);

            if (brace_open->type != next->type)                       // Issue #2836
            {
               newline_add_between(brace_open, chunk_get_next_nc_nnl(brace_open));
            }
            // Make sure nothing is cuddled with the closing brace
            pc = chunk_get_next_type(brace_open, CT_BRACE_CLOSE, brace_open->level);
            newline_add_between(pc, chunk_get_next_nc_nnl_nb(pc));
            retval = true;
         }
      }
   }
...
static void newlines_if_for_while_switch_pre_blank_lines(Chunk *start, iarf_e nl_opt)
{
    ...
    */
   for (Chunk *pc = start->get_prev(); pc != nullptr && pc->IsNotNullChunk(); pc = pc->get_prev())
   {
      ...
      else
      {
         ...
         if (do_add) // we found something previously besides a comment or a new line
         {
            ...
            else
            {
               ...
               if (  ((next = pc->get_next())->IsNotNullChunk())
                  && chunk_is_comment(next))
               {
                  pc = next;
               }
...
static void newlines_func_pre_blank_lines(Chunk *start, c_token_t start_type)
{
   ...
   for (pc = start->get_prev(); pc != nullptr && pc->IsNotNullChunk(); pc = pc->get_prev())
   {
...
static Chunk *get_closing_brace(Chunk *start)
{
   ...
   for (pc = start; (pc = pc->get_next())->IsNotNullChunk();)
   {
...
static void remove_next_newlines(Chunk *start)
{
   ...
   while ((next = start->get_next())->IsNotNullChunk())
   {
      if (  chunk_is_newline(next)
...
static void newlines_enum(Chunk *start)
{
   ...
   // look for 'enum class'
   Chunk *pcClass = chunk_get_next_nc_nnl(start);
   ...
   if (chunk_is_token(pcClass, CT_ENUM_CLASS))
   {
      ...
      Chunk *pcType = chunk_get_next_nc_nnl(pcClass);
      ...
      if (chunk_is_token(pcType, CT_TYPE))
      {
         ...
         // look for ':'
         Chunk *pcColon = chunk_get_next_nc_nnl(pcType);
         ...
         if (chunk_is_token(pcColon, CT_BIT_COLON))
         {
            ...
            Chunk *pcType1 = chunk_get_next_nc_nnl(pcColon);
            ...
            if (chunk_is_token(pcType1, CT_TYPE))
            {
               ...
               Chunk *pcType2 = chunk_get_next_nc_nnl(pcType1);
...
static void newlines_namespace(Chunk *start)
{
   ...
   Chunk *beforeBrace = braceOpen->get_prev();
...
static void newlines_cuddle_uncuddle(Chunk *start, iarf_e nl_opt)
{
   ...
   Chunk *br_close = chunk_get_prev_nc_nnl_ni(start);   // Issue #2279
...
static void newlines_do_else(Chunk *start, iarf_e nl_opt)
{
   ...
   Chunk *next = chunk_get_next_nc_nnl(start);
         ...
         || chunk_is_token(next, CT_VBRACE_OPEN)))
   {
      ...
      if (chunk_is_token(next, CT_VBRACE_OPEN))
      {
         ...
         if (nl_opt & IARF_ADD)
         {
            newline_iarf_pair(start, chunk_get_next_nc_nnl(next), nl_opt);
      ...
      else
      {
         ...
         newline_add_between(next, chunk_get_next_nc_nnl(next));
      }
   }
} // newlines_do_else
...
static bool collapse_empty_body(Chunk *br_open)
{
   ...
   if (  !options::nl_collapse_empty_body()
      || !chunk_is_token(br_open->get_next_nnl(), CT_BRACE_CLOSE))
   {
      return(false);
   }

   for (Chunk *pc = br_open->get_next()
...
static void newlines_brace_pair(Chunk *br_open)
{
      ...
      && !br_open->flags.test(PCF_NOT_POSSIBLE))          // Issue #2795
   {
         ...
         && chunk_is_paren_close(tmp))                     // need to check the conditions.
      {
         ...
         while (  tmp != nullptr
               && (tmp = tmp->get_next())->IsNotNullChunk()
         ...
         if (is_it_possible)
         {
            ...
            if (options::code_width() > 0)
            {
               ...
               Chunk *current       = chunk_get_prev_nc_nnl_ni(br_open);
               Chunk *next_br_close = br_close->get_next();
               ...
               while (current->IsNotNullChunk())
               {
                  ...
                  Chunk *the_next = current->get_next();
            ...
            Chunk *tmp_1 = chunk_get_prev_nc_nnl_ni(br_open);

            while (  tmp_1 != nullptr
                  && tmp_1->IsNotNullChunk()
                  && (tmp_1 = tmp_1->get_next())->IsNotNullChunk()
                  && !chunk_is_closing_brace(tmp_1)
                  && (tmp_1->get_next()->IsNotNullChunk()))
            {
               ...
               {
                  tmp_1 = tmp_1->get_prev();                 // Issue #1825
                  newline_iarf_pair(tmp_1, chunk_get_next_nc_nnl(tmp_1), IARF_REMOVE);
               }
            }
   ...
   Chunk *next = br_open->get_next_nc();
   ...
   if (get_chunk_parent_type(br_open) == CT_ASSIGN)
   {
      ...
      if (chunk_is_newline(next))
      {
         Chunk *prev = chunk_get_prev_nc_nnl_ni(br_open);   // Issue #2279
      ...
      || get_chunk_parent_type(br_open) == CT_FUNC_CALL_USER)
   {
      ...
      else
      {
            ...
            || get_chunk_parent_type(br_open) == CT_OC_CLASS)
         {
            ...
            {
               prev = chunk_get_prev_nc_nnl_ni(br_open);   // Issue #2279
   ...
   Chunk *prev = chunk_get_prev_nc_nnl_nb(br_close);
...
static void newline_after_return(Chunk *start)
{
   ...
   Chunk *after = chunk_get_next_nc_nnl_nb(semi);
   ...
   for (pc = semi->get_next(); pc != after; pc = pc->get_next())
   {
      if (chunk_is_token(pc, CT_NEWLINE))
      {
         if (pc->nl_count < 2)
...
static void newline_func_multi_line(Chunk *start)
{
   ...
   {
      Chunk *start_next         = chunk_get_next_nc_nnl(start);
      ...
      if (  add_start
         && !chunk_is_newline(start->get_next()))
      {
         ...
         && !chunk_is_newline(pc->get_prev()))
      {
         ...
         if (options::nl_func_call_args_multi_line_ignore_closures())
         {
            if (  !has_leading_closure
               && !has_trailing_closure)
            {
               newline_iarf(pc->get_prev(), IARF_ADD);
            }
         }
         else
         {
      ...
      if (add_args)
      {
         ...
         for (pc = chunk_get_next_nc_nnl(start);
              ...
              pc = chunk_get_next_nc_nnl(pc))
         {
               ...
               && (pc->level == (start->level + 1)))
            {
               ...
               if (!chunk_is_newline(pc->get_next()))
               {
                  log_rule_B("nl_func_call_args_multi_line_ignore_closures");
                  ...
                  if (options::nl_func_call_args_multi_line_ignore_closures())
                  {
                     ...
                     Chunk *after_comma = chunk_get_next_nc_nnl(pc);
...
static void newline_template(Chunk *start)
{
   ...
   if (chunk_is_token(pc, CT_ANGLE_CLOSE))
   {
      ...
      if (add_args)
      {
         ...
         for (pc_1 = chunk_get_next_nc_nnl(start);
              pc_1 != nullptr && pc_1->level > start->level;
              pc_1 = chunk_get_next_nc_nnl(pc_1))
         {
            if (  chunk_is_token(pc_1, CT_COMMA)
               && (pc_1->level == (start->level + 1)))
            {
               Chunk *tmp = pc_1->get_next();
               ...
               if (!chunk_is_newline(pc_1->get_next()))
               {
...
static void newline_func_def_or_call(Chunk *start)
{
   ...
   else
   {
      ...
      prev = chunk_is_paren_close(prev) ? nullptr : chunk_get_prev_nc_nnl_ni(prev);   // Issue #2279
      ...
      {
         newline_iarf(chunk_get_prev_nc_nnl_ni(prev), options::nl_func_class_scope());   // Issue #2279
      }
      ...
      if (chunk_is_not_token(prev, CT_ACCESS_COLON))
      {
         ...
         {
            tmp  = prev;
            prev = chunk_get_prev_nc_nnl_ni(prev);   // Issue #2279
         }
         else
         {
            tmp = start;
         ...
         const Chunk *tmp_next = chunk_get_next_nc_nnl(prev);

         if (chunk_is_not_token(tmp_next, CT_FUNC_CLASS_DEF))
         {
            Chunk  *closing = chunk_skip_to_match(tmp);
            Chunk  *brace   = chunk_get_next_nc_nnl(closing);
            iarf_e a;                                            // Issue #2561
      ...
      if (chunk_is_str(pc, ")", 1))
      {
         ...
         if (atmp != IARF_IGNORE)
         {
            prev = chunk_get_prev_nc_nnl_ni(start);   // Issue #2279
...
static void newline_oc_msg(Chunk *start)
{
   ...
   for (Chunk *pc = chunk_get_next_nc_nnl(start); pc; pc = chunk_get_next_nc_nnl(pc))
   {
...
void undo_one_liner(Chunk *pc)
{
      ...
      && pc->flags.test(PCF_ONE_LINER))
   {
      ...
      while ((tmp = tmp->get_prev())->IsNotNullChunk())
      {
...
static void nl_create_one_liner(Chunk *vbrace_open)
{
   ...
   Chunk *tmp   = chunk_get_next_nc_nnl(vbrace_open);
   Chunk *first = tmp;
...
void newlines_cleanup_braces(bool first)
{
   ...
   for ( ; pc != nullptr && pc->IsNotNullChunk(); pc = chunk_get_next_nc_nnl(pc))
   {
      ...
      else if (chunk_is_token(pc, CT_TRY))
      {
         ...
         // Issue #1734
         Chunk *po = chunk_get_next_nc_nnl(pc);
      ...
      else if (chunk_is_token(pc, CT_BRACE_OPEN))
      {
         ...
         case CT_OC_CLASS:
         {
            if (pc->level == pc->brace_level)
            {
               ...
               for (Chunk *tmp = pc->get_prev(); tmp->IsNotNullChunk(); tmp = tmp->get_prev())
               {
                  LOG_FMT(LBLANK, "%s(%d): orig_line is %zu, orig_col is %zu, text() is '%s'\n",
                          __func__, __LINE__, tmp->orig_line, tmp->orig_col, tmp->text());
         ...
         case CT_OC_BLOCK_EXPR:
         {
            ...
            newline_iarf_pair(pc->get_prev(), pc, options::nl_oc_block_brace());
            break;
         }
         ...
         case CT_FUNC_CLASS_DEF:                             // Issue #2343
         {
            ...
            else
            {
               ...
               {
                  newline_iarf_pair(pc->get_prev(), pc, options::nl_before_opening_brace_func_class_def());
               }
            }
         }
         ...
         else
         {
            ...
            if (get_chunk_parent_type(pc) == CT_BRACED_INIT_LIST)
            {
               ...
               newline_iarf_pair(pc, pc->get_next_nnl(),
      ...
      else if (chunk_is_token(pc, CT_VBRACE_OPEN))
      {
               ...
               && options::nl_split_while_one_liner()))
         {
            ...
            {
               // split one-liner
               Chunk *end = chunk_get_next_type(pc->get_next(), CT_SEMICOLON, -1)->get_next();
               // Scan for clear flag
               LOG_FMT(LNEWLINE, "(%d) ", __LINE__);
               LOG_FMT(LNEWLINE, "\n");

               for (Chunk *temp = pc; temp != end; temp = temp->get_next())
               {
      ...
      else if (chunk_is_token(pc, CT_VBRACE_CLOSE))
      {
         ...
         {
            if (!chunk_is_newline(pc->get_next_nc()))
            {
               newline_iarf(pc, IARF_ADD);
            }
         }
      ...
      else if (chunk_is_token(pc, CT_THROW))
      {
            ...
            || chunk_is_token(prev, CT_FPAREN_CLOSE))         // Issue #1122
         {
            ...
            newline_iarf(chunk_get_prev_nc_nnl_ni(pc), options::nl_before_throw());   // Issue #2279
         }
      }
      ...
      else if (chunk_is_token(pc, CT_ANGLE_CLOSE))
      {
         if (get_chunk_parent_type(pc) == CT_TEMPLATE)
         {
            ...
            {
               Chunk *tmp = chunk_get_prev_nc_nnl_ni(chunk_get_prev_type(pc, CT_ANGLE_OPEN, pc->level));   // Issue #2279
      ...
      else if (chunk_is_token(pc, CT_SQUARE_OPEN))
      {
         if (  get_chunk_parent_type(pc) == CT_ASSIGN
            && !pc->flags.test(PCF_ONE_LINER))
         {
            Chunk *tmp = chunk_get_prev_nc_nnl_ni(pc);   // Issue #2279
...
static void nl_handle_define(Chunk *pc)
{
   ...
   while ((nl = nl->get_next())->IsNotNullChunk())
   {
   ...
   for (Chunk *pc = Chunk::GetHead(); pc != nullptr && pc->IsNotNullChunk(); pc = chunk_get_next_nc_nnl(pc))
   {
      ...
      if (chunk_is_token(pc, chunk_type))
      {
         ...
         if (nl_flag == 3)
         {
            ...
            {
               remove_next_newlines(chunk_get_prev_nc_nnl_ni(pc));   // Issue #2279
            }
            continue;
         }
            ...
            && pc->level == ccolon->level)
         {
            ...
            if (ncia & IARF_ADD)                   // nl_class_init_args, nl_constr_init_args:
            {
               if (pcc & TP_TRAIL)                 // pos_class_comma, pos_constr_comma
               {
                  ...
                  {
                     Chunk *after = pc->get_next();   // Issue #2759
...
void do_blank_lines(void)
{
   ...
   for (Chunk *pc = Chunk::GetHead(); pc != nullptr && pc->IsNotNullChunk(); pc = pc->get_next())
   {
      if (chunk_is_token(pc, CT_NEWLINE))
      {
         LOG_FMT(LBLANKD, "%s(%d): orig_line is %zu, orig_col is %zu, <Newline>, nl is %zu\n",
         ...
         && pc->level > open_brace->level)
   {
      if (  (pc->level != (open_brace->level + 1))
         ...
         || (  chunk_is_token(pc, CT_COMMA)
            ...
            && (  pc->get_next()->type == CT_COMMENT_CPP
               || pc->get_next()->type == CT_COMMENT)))
      {
         continue;
      }
      newline_iarf(pc, av);
...
void annotations_newlines(void)
{
   ...
   while (  (pc = chunk_get_next_type(pc, CT_ANNOTATION, -1)) != nullptr
         && (next = pc->get_next_nnl())->IsNotNullChunk())
   {
...
bool newlines_between(Chunk *pc_start, Chunk *pc_end, size_t &newlines, E_Scope scope)
{
   ...
   for ( ; it->IsNotNullChunk() && it != pc_end; it = it->get_next(scope))
   {
