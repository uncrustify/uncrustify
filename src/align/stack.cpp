...
void AlignStack::Add(Chunk *start, size_t seqnum)
{
   ...
   if (m_star_style != SS_IGNORE)
   {
      ...
      if (tmp_prev->Is(E_Token::TPAREN_OPEN))
      {
         ali      = tmp_prev;
         tmp_prev = ali->GetPrev();
...
void AlignStack::Flush()
{
   ...
   if (Len() == 1)
   {
      ...
      Chunk *temp = pc->GetPrevType(E_Token::TYPEDEF, pc->GetLevel());
   ...
   for (size_t idx = 0; idx < Len(); idx++)
   {
      ...
      {
         Chunk *tmp = (pc->Is(E_Token::TPAREN_OPEN)) ? pc->GetNext() : pc;
      ...
      if (m_right_align)
      {
         ...
         if (pc->GetAlignData().start->IsNotNullChunk())
         {
            ...
            if (pc->GetAlignData().start->GetType() == E_Token::NEG)
            {
