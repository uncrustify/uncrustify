...
void align_add(ChunkStack &cs, Chunk *pc, size_t &max_col)
{
   ...
   {
      if (prev->Is(E_Token::COMMENT_MULTI))
      {
      ...
      LOG_FMT(LALADD, "%s(%d): pc orig line=%zu, pc->col=%zu max_col=%zu min_col=%zu multi:%s prev->col=%zu prev->Len()=%zu %s\n",
              __func__, __LINE__, pc->GetOrigLine(), pc->GetColumn(), max_col, min_col, (prev->Is(E_Token::COMMENT_MULTI)) ? "Y" : "N",
              (prev->Is(E_Token::COMMENT_MULTI)) ? prev->GetOrigColEnd() : prev->GetColumn(), prev->Len(), get_token_name(prev->GetType()));
   }
