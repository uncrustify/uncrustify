...
Chunk *align_assign(Chunk *first, size_t span, size_t thresh, bool parent_is_enum, size_t *p_nl_count)
{
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      if (  pc->Is(E_Token::SPAREN_OPEN)
            // || pc->Is(E_Token::FPAREN_OPEN) Issue #1340
         ...
         && !(pc->GetParentType() == E_Token::BRACED_INIT_LIST))
      {
         size_t     myspan;
         size_t     mythresh;
         const bool myparent_is_enum = pc->GetParentType() == E_Token::ENUM;
              ...
              && !options::align_assign_on_multi_var_defs())
      {
         ...
         vdas_pc = Chunk::NullChunkPtr;
              ...
              && (  pc->Is(E_Token::ASSIGN)
                 || pc->Is(E_Token::ASSIGN_DEFAULT_ARG)
                 || pc->Is(E_Token::ASSIGN_FUNC_PROTO)))
      {
         if (pc->Is(E_Token::ASSIGN))               // Issue #2236
         {
            equ_count++;
         }
         ...
         if (  options::align_assign_decl_func() == 0  // Align with other assignments (default)
            && (  pc->Is(E_Token::ASSIGN_DEFAULT_ARG)  // Foo( int bar = 777 );
               || pc->Is(E_Token::ASSIGN_FUNC_PROTO))) // Foo( const Foo & ) = delete;
         {
         ...
         else if (options::align_assign_decl_func() == 1)   // Align with each other
         {
            ...
            if (pc->Is(E_Token::ASSIGN_DEFAULT_ARG))  // Foo( int bar = 777 );
            {
            ...
            else if (pc->Is(E_Token::ASSIGN_FUNC_PROTO))  // Foo( const Foo & ) = delete;
            {
            ...
            else if (pc->Is(E_Token::ASSIGN)) // Issue #2197
            {
               vdas_pc     = pc;
