...
comment_align_e get_comment_align_type(Chunk *cmt)
{
   ...
   {
      if (  prev->Is(E_Token::PP_ENDIF)
         || prev->Is(E_Token::PP_ELSE)
         || prev->Is(E_Token::ELSE)
         || prev->Is(E_Token::BRACE_CLOSE))
      {
         // TODO: make the magic 3 configurable
         if ((cmt->GetColumn() - (prev->GetColumn() + prev->Len())) < 3)
         {
            cmt_type = (prev->Is(E_Token::PP_ENDIF)) ? comment_align_e::ENDIF : comment_align_e::BRACE;
         }
      }
   }
   return(cmt_type);
...
void align_right_comments()
{
   ...
   {
      if (  pc->Is(E_Token::COMMENT)
         || pc->Is(E_Token::COMMENT_CPP)
         || pc->Is(E_Token::COMMENT_MULTI))
      {
         if (pc->GetParentType() == E_Token::COMMENT_END)
         {
         ...
         if (pc->GetParentType() == E_Token::COMMENT_WHOLE)
         {
