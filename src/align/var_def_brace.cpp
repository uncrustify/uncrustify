...
Chunk *align_var_def_brace(Chunk *start, size_t span, size_t *p_nl_count)
{
   ...
   if (  start->GetParentType() == E_Token::STRUCT
      || start->GetParentType() == E_Token::UNION)
   {
   ...
   else if (start->GetParentType() == E_Token::CLASS)
   {
   ...
   if (prev->Is(E_Token::ASSIGN))
   {
      ...
      Chunk *pc = start->GetNextType(E_Token::BRACE_CLOSE, start->GetLevel());
   ...
   while (pc->IsNotNullChunk())
   {
         ...
         && !pc->TestFlags(PCF_IN_CLASS_BASE))
      {
               ...
               && options::align_single_line_func()))
         {
            ...
            fp_look_bro = (pc->Is(E_Token::FUNC_DEF))
      ...
      if (  !pc->TestFlags(PCF_IN_CLASS_BASE)
         && pc->IsNot(E_Token::FUNC_CLASS_DEF)
         && pc->IsNot(E_Token::FUNC_CLASS_PROTO)
         && ((pc->GetFlags() & align_mask) == PCF_VAR_1ST)
         && pc->IsNot(E_Token::FUNC_DEF)                                    // Issue 1452
         && (  (pc->GetLevel() == (start->GetLevel() + 1))
            || pc->GetLevel() == 0)
         && pc->GetPrev()->IsNot(E_Token::MEMBER))
      {
         ...
         if (!did_this_line)
         {
               ...
               && (as.m_star_style == AlignStack::SS_INCLUDE))
            {
               ...
               while (  prev_local->Is(E_Token::PTR_TYPE)
                     || prev_local->Is(E_Token::ADDR))
               {
            ...
            if (prev_local->IsNot(E_Token::DEREF))                // Issue #2971
            {
               LOG_FMT(LAVDB, "%s(%d): add = '%s', orig line is %zu, orig col is %zu, level is %zu\n",
                       __func__, __LINE__, pc->GetLogText(), pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLevel());
      ...
      else if (pc->Is(E_Token::BIT_COLON))
      {
         if (!did_this_line)
         {
            as_bc.Add(pc);
