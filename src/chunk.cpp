...
static void chunk_log_msg(const Chunk *chunk, const log_sev_t log, const char *str)
{
   ...
   if (chunk->Is(E_Token::NEWLINE))
   {
      LOG_FMT(log, "<Newline>,\n");
   }
   else if (chunk->Is(E_Token::VBRACE_OPEN))
   {
      LOG_FMT(log, "<VBRACE_OPEN>,\n");
   }
   else if (chunk->Is(E_Token::VBRACE_CLOSE))
   {
...
bool Chunk::IsAddress() const
{
   if (  IsNotNullChunk()
      && (  Is(E_Token::BYREF)
         || (  Len() == 1
            && m_text[0] == '&'
            && IsNot(E_Token::OPERATOR_VAL))))
   {
      const Chunk *prevc = GetPrev();

      if (  TestFlags(PCF_IN_TEMPLATE)
         && (  prevc->Is(E_Token::COMMA)
            || prevc->Is(E_Token::ANGLE_OPEN)))
      {
         return(false);
      }
      return(true);
...
void Chunk::SetType(const E_Token token)
{
   ...
   if (token == E_Token::NEWLINE)
   {
      LOG_FMT(LSETTYP, "<newline (NL)>");
      Text() = "\n";
   }
   else if (token == E_Token::NL_CONT)
   {
      LOG_FMT(LSETTYP, "<newline (NL CONT)>");
      Text() = "\\\n";
   }
   else if (token == E_Token::WHITESPACE)
   {
...
Chunk *Chunk::SkipDcMember() const
{
   ...
   Chunk       *pc  = const_cast<Chunk *>(this);
   const Chunk *nxt = pc->Is(E_Token::DC_MEMBER) ? pc : pc->GetNextNcNnl(E_Scope::ALL);

   while (nxt->Is(E_Token::DC_MEMBER))
   {
...
bool Chunk::IsOCForinOpenParen() const
{
   if (  language_is_set(lang_flag_e::LANG_OC)
      && Is(E_Token::SPAREN_OPEN)
      && GetPrevNcNnl()->Is(E_Token::FOR))
   {
      const Chunk *nxt = const_cast<Chunk *>(this);

      while (  nxt->IsNotNullChunk()
            && nxt->IsNot(E_Token::SPAREN_CLOSE)
            && nxt->IsNot(E_Token::IN))
      {
         nxt = nxt->GetNextNcNnl();
      }

      if (nxt->Is(E_Token::IN))
      {
         return(true);
      }
   }
...
{
   if (  Is(E_Token::PAREN_OPEN)
      || Is(E_Token::SPAREN_OPEN)
      || Is(E_Token::FPAREN_OPEN)
      || Is(E_Token::TPAREN_OPEN)
      || Is(E_Token::BRACE_OPEN)
      || Is(E_Token::VBRACE_OPEN)
      || Is(E_Token::ANGLE_OPEN)
      || Is(E_Token::SQUARE_OPEN))
   {
...
{
   if (  Is(E_Token::PAREN_CLOSE)
      || Is(E_Token::SPAREN_CLOSE)
      || Is(E_Token::FPAREN_CLOSE)
      || Is(E_Token::TPAREN_CLOSE)
      || Is(E_Token::BRACE_CLOSE)
      || Is(E_Token::VBRACE_CLOSE)
      || Is(E_Token::ANGLE_CLOSE)
      || Is(E_Token::SQUARE_CLOSE))
   {
...
{
   return(  Is(E_Token::ACCESS_COLON)
         || Is(E_Token::ASM_COLON)
         || Is(E_Token::BIT_COLON)
         || Is(E_Token::CASE_COLON)
         || Is(E_Token::CLASS_COLON)
         || Is(E_Token::COLON)
         || Is(E_Token::COND_COLON)
         || Is(E_Token::CONSTR_COLON)
         || Is(E_Token::CS_SQ_COLON)
         || Is(E_Token::D_ARRAY_COLON)
         || Is(E_Token::ENUM_COLON)
         || Is(E_Token::FOR_COLON)
         || Is(E_Token::LABEL_COLON)
         || Is(E_Token::OC_COLON)
         || Is(E_Token::OC_DICT_COLON)
         || Is(E_Token::TAG_COLON)
         || Is(E_Token::WHERE_COLON));
} // Chunk::IsColon
...
{
   return(  Is(E_Token::TYPE)
         || Is(E_Token::PTR_TYPE)
         || Is(E_Token::BYREF)
         || Is(E_Token::DC_MEMBER)
         || Is(E_Token::QUALIFIER)
         || Is(E_Token::STRUCT)
         || Is(E_Token::ENUM)
         || Is(E_Token::UNION));
