...
void remove_extra_returns()
{
   ...
   {
      LOG_FMT(LRMRETURN, "%s(%d): orig line is %zu, orig col is %zu, text is '%s', len is %zu, type is %s, parent type is %s\n",
         ...
         && !pc->TestFlags(PCF_IN_PREPROC))
      {
         ...
         Chunk *closing_brace = pc->GetNextType(E_Token::CT_BRACE_CLOSE, 1);
         ...
         {
            if (closing_brace->GetParentType() == E_Token::CT_FUNC_CLASS_DEF)
            {
               // we have a class. Do nothing
            }
            else if (  closing_brace->GetParentType() == E_Token::CT_FUNC_DEF
                    && pc->GetLevel() < 2)
            {
               remove_it = true;
            }
         ...
         else
         {
            ...
            closing_brace = pc->GetNextType(E_Token::CT_BRACE_CLOSE, 0);
            LOG_FMT(LRMRETURN, "%s(%d): on orig line %zu, level is %zu\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetLevel());

            if (closing_brace->IsNotNullChunk())
            {
               if (  closing_brace->GetParentType() == E_Token::CT_FUNC_DEF
                  && pc->GetLevel() < 2)
               {
                  remove_it = true;
               }
         ...
         if (remove_it)
         {
            ...
            if (  semicolon->IsNotNullChunk()
               && semicolon->Is(E_Token::CT_SEMICOLON))
            {
