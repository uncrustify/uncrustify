...
void fix_typedef(Chunk *start)
{
        ...
        ; next = next->GetNextNcNnl(E_Scope::PREPROC))
   {
      ...
      if (start->level == next->level)
      {
         ...
         next->ResetFlags(PCF_VAR_1ST_DEF);
...
void mark_cpp_constructor(Chunk *pc)
{
         ...
         && !tmp->IsSemicolon())
   {
      ...
      tmp->SetFlags(PCF_IN_CONST_ARGS);
...
void mark_function(Chunk *pc)
{
      ...
      && tmp->IsString("("))
   {
            ...
            || tmp2->Is(CT_WORD)))
      {
         ...
         pc->ResetFlags(PCF_VAR_1ST_DEF);

         if (tmp2->IsNotNullChunk())
         {
            tmp2->SetFlags(PCF_VAR_1ST_DEF);
         }
         ...
         || (pc->Is(CT_FUNC_PROTO))))
   {
      ...
      while (  tmp->IsNotNullChunk()
            ...
            && tmp->IsNot(CT_SEMICOLON))
      {
         mark_where_chunk(tmp, pc->GetType(), tmp->flags | in_where_spec_flags);
   ...
   if (pc->Is(CT_FUNC_DEF))
   {
            ...
            && tmp->IsNot(CT_BRACE_OPEN))
      {
         ...
         if (!tmp->IsSemicolon())
         {
            tmp->SetFlags(PCF_OLD_FCN_PARAMS);
         }
         tmp = tmp->GetNextNcNnl();
      }
...
bool mark_function_type(Chunk *pc)
{
   ...
   if (!anon)
   {
      ...
      else
      {
         ...
         varcnk->SetFlags(PCF_VAR_1ST_DEF);
      }
   }
   ...
   if (aft->IsSemicolon())
   {
      aft->SetParentType(aft->flags.test(PCF_IN_TYPEDEF) ? CT_TYPEDEF : CT_FUNC_VAR);
   }
   ...
   while ((tmp = tmp->GetPrevNcNnlNi())->IsNotNullChunk()) // Issue #2279
   {
      ...
      if (*tmp->str.c_str() == '(')
      {
         ...
         {
            tmp->SetType(CT_TYPE);
            tmp->ResetFlags(PCF_VAR_1ST_DEF);
         }
...
void mark_template_func(Chunk *pc, Chunk *pc_next)
{
   ...
   if (after->IsNotNullChunk())
   {
      ...
      else if (after->Is(CT_WORD))
      {
         // its a type!
         pc->SetType(CT_TYPE);
         pc->SetFlags(PCF_VAR_TYPE);
         after->SetFlags(PCF_VAR_DEF);
      }
   }
} // mark_template_func
   ...
   if (var_name != nullptr)
   {
      ...
      while ((word_type = cs.Pop_Back()) != nullptr)
      {
            ...
            || word_type->Is(CT_TYPE))
         {
            ...
            word_type->SetFlags(PCF_VAR_TYPE);
         }
         word_cnt++;
      }
      ...
      if (var_name->Is(CT_WORD))
      {
         ...
         else
         {
            ...
            var_name->SetFlags(PCF_VAR_TYPE);
         }
      }
   }
} // mark_variable_stack
