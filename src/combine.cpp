           ...
           || pc->type == CT_TYPE))
      {
             ...
             )
         {
            ...
            if (brace_close != nullptr
                && brace_close->type == CT_BRACE_CLOSE)
            {
               pc->type                 = CT_TYPE;
   ...
   while (next != nullptr)
   {
                   ...
                   && cs_top_is_question(cs, next->level)))
      {
         ...
         else
         {
            ...
            else if ((cur->type == CT_ANGLE_CLOSE)
                     || hit_class)
            {
...
static bool can_be_full_param(chunk_t *start, chunk_t *end)
{
   ...
   for (pc = start; (pc != nullptr)
        && (pc != end); pc = chunk_get_next_ncnl(pc, scope_e::PREPROC))
   {
       ...
       && chunk_is_str(tmp, "(", 1))
   {
      ...
      while (tmp1 != nullptr)
      {
         ...
         if (!chunk_is_word(tmp1)
             || !chunk_is_token(tmp2, CT_DC_MEMBER))
         {
            break;
         }
...
static void handle_cpp_template(chunk_t *pc)
{
   ...
   while ((tmp = chunk_get_next(tmp)) != nullptr)
   {
      ...
      else if ((tmp->type == CT_ANGLE_CLOSE)
               && (tmp->level == level))
      {
         set_chunk_parent(tmp, CT_TEMPLATE);
         break;
...
{
   if ((ang_close != nullptr)
       && (ang_close->type == CT_ANGLE_CLOSE))
   {
...
static void handle_oc_property_decl(chunk_t *os)
{
   if (cpd.settings[UO_mod_sort_oc_properties].b)
   {
      ...
      if (next->type == CT_PAREN_OPEN)
      {
         ...
         while (curr_chunk
                && curr_chunk->type != CT_PAREN_CLOSE)
         {
