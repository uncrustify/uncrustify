...
static bool for_needs_rewrite(Chunk *pc, E_Token desired_type)
{
   ...
   if (desired_type == E_Token::CT_FOR)
   {
      return(false);
   }
   pc = pc->GetNext();
...
static bool while_needs_rewrite(Chunk *keyword, E_Token desired_type, const char *desired_condition)
{
   ...
   if (!cparen->Is(E_Token::CT_SPAREN_CLOSE))
   {
      return(false);
   }
...
static void move_one_token(Chunk * &source, Chunk * &destination, E_Token parent_type)
{
   ...
   source->SetColumn(destination->GetColumnEnd());
                                   ...
                                   E_Token desired_type, const char *desired_condition)
{
   ...
   if (desired_type == E_Token::CT_FOR)
   {
      source->SetType(E_Token::CT_SEMICOLON);
      source->SetParentType(E_Token::CT_FOR);
      source->Text() = ";";
      move_one_token(source, destination, desired_type);
...
void rewrite_infinite_loops()
{
   ...
   {
   case 1: // for(;;)
      desired_type      = E_Token::CT_FOR;
      desired_condition = nullptr;
      break;

   case 2: // while(true)
      desired_type      = E_Token::CT_WHILE;
      desired_condition = "true";
      break;

   case 3: // do...while(true)
      desired_type      = E_Token::CT_WHILE_OF_DO;
      desired_condition = "true";
      break;
   ...
   for (Chunk *pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnl())
   {
      if (pc->Is(E_Token::CT_DO))
      {
         ...
         if (  !while_keyword->Is(E_Token::CT_WHILE_OF_DO)
            || !while_needs_rewrite(while_keyword, desired_type, desired_condition))
         {
            continue;
         }

         if (desired_type == E_Token::CT_WHILE_OF_DO)
         {
            ...
            start_brace->SetParentType(E_Token::CT_DO);
            end_brace->SetParentType(E_Token::CT_DO);
         }
         else
         {
            Chunk *top    = pc;
                 ...
                 && for_needs_rewrite(pc, desired_type)))
      {
         ...
         if (desired_type == E_Token::CT_WHILE_OF_DO)
         {
            ...
            if (bottom->Is(E_Token::CT_VBRACE_CLOSE))
            {
            ...
            rewrite_loop_keyword(bottom, E_Token::CT_WHILE_OF_DO);

            // Change the 'while' at the top of the loop to a 'do'
            rewrite_loop_keyword(top, E_Token::CT_DO);
            top = top->GetNext();
            ...
            bottom->SetType(E_Token::CT_SEMICOLON);
            bottom->Text() = ";";
