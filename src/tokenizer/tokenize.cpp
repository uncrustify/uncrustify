...
static bool parse_comment(TokenContext &ctx, Chunk &pc)
{
   ...
   if (ch == '/')          // 47
   {
      pc.SetType(E_Token::COMMENT_CPP);
   ...
   else if (ch == '+')                         // 43
   {
            ...
            && ctx.more())
      {
         ...
         if (  (ch == '\n')
            || (ch == '\r'))
         {
            pc.SetType(E_Token::COMMENT_MULTI);
            pc.SetNlCount(pc.GetNlCount() + 1);
...
static bool parse_number(TokenContext &ctx, Chunk &pc)
{
   ...
   pc.SetType(is_float ? E_Token::NUMBER_FP : E_Token::NUMBER);
...
static bool parse_string(TokenContext &ctx, Chunk &pc, size_t quote_idx, bool allow_escape)
{
   ...
   while (ctx.more())
   {
      ...
      {
         pc.SetNlCount(pc.GetNlCount() + 1);
         pc.SetType(E_Token::STRING_MULTI);
      }
      else if (  ch == '\r'
              && ctx.peek() != '\n')
      {
...
static bool parse_word(TokenContext &ctx, Chunk &pc, bool skipcheck)
{
   ...
   if (  cpd.in_preproc == E_Token::PP_DEFINE
      && cpd.preproc_ncnl_count == 1)
   {
      if (ctx.peek() == '(')               // 40
      {
         pc.SetType(E_Token::MACRO_FUNC);
      }
      else
      {
         pc.SetType(E_Token::MACRO);
         ...
         if (options::pp_ignore_define_body())
         {
             ...
             */
            cpd.in_preproc = E_Token::PP_IGNORE;
         }
      }
   }
   else
   {
      ...
      else
      {
          ...
          * substitution. */
         if (  pc.GetType() == E_Token::PP_IGNORE
            && !cpd.in_preproc)
         {
            pc.SetType(find_keyword_type(pc.GetLogText(), pc.GetText().size()));
         }
         else if (pc.GetType() == E_Token::COMMENT_CPP)   // Issue #1460
         {
            size_t ch;
            bool   is_cs = language_is_set(lang_flag_e::LANG_CS);
...
static bool parse_whitespace(TokenContext &ctx, Chunk &pc)
{
   ...
   {
      pc.Text().clear();
      pc.SetType(nl_count ? E_Token::NEWLINE : E_Token::WHITESPACE);
...
static bool parse_macro(TokenContext &ctx, Chunk &pc, const Chunk *prev_pc)
{
   ...
   if (parse_comment(ctx, pc))  // allow E_Token::COMMENT_MULTI within macros
   {
      return(true);
   }
   ctx.save();
   ...
   bool continued = (  prev_pc->Is(E_Token::NL_CONT)
                    || prev_pc->Is(E_Token::COMMENT_MULTI));
...
static bool parse_next(TokenContext &ctx, Chunk &pc, const Chunk *prev_pc)
{
   ...
   // If it is turned off, we put everything except newlines into E_Token::UNKNOWN
   if (cpd.unc_off)
   {
   ...
   if (  cpd.in_preproc > E_Token::PP_BODYCHUNK
      && cpd.in_preproc <= E_Token::PP_OTHER)
   {
      pc.Text().clear();
      TokenInfo ss;
      ctx.save(ss);
      // Chunk to a newline or comment
      pc.SetType(E_Token::PREPROC_BODY);
      size_t last = 0;
   ...
   if (language_is_set(lang_flag_e::LANG_PAWN))
   {
      if (  cpd.preproc_ncnl_count == 1
         && (  cpd.in_preproc == E_Token::PP_DEFINE
            || cpd.in_preproc == E_Token::PP_EMIT))
      {
         parse_pawn_pattern(ctx, pc, E_Token::MACRO);
         return(true);
      }
   ...
   else
   {
      ...
      if (  (  (  (ch == 'L')            // 76
         ...
         || (ch == '\'')                 // 39
         || (  (ch == '<')               // 60
            && cpd.in_preproc == E_Token::PP_INCLUDE))
      {
         parse_string(ctx, pc, unc_isalpha(ch) ? 1 : 0, true);

         if (cpd.in_preproc == E_Token::PP_INCLUDE)
         {
            pc.SetParentType(E_Token::PP_INCLUDE);
         }
         return(true);
      }
      ...
      if (  (ch == '<')                    // 60
         && cpd.in_preproc == E_Token::PP_DEFINE)
      {
         if (Chunk::GetTail()->Is(E_Token::MACRO))
         {
      ...
      if (  (ch == '(')                 // 40
         && (tail->IsNotNullChunk())
         && (  tail->Is(E_Token::CNG_HASINC)
            || tail->Is(E_Token::CNG_HASINCN)))
      {
         parse_string(ctx, pc, 0, false);
         return(true);
      }
...
void tokenize(const deque<int> &data, Chunk *ref)
{
   ...
   while (ctx.more())
   {
      ...
      if (  language_is_set(lang_flag_e::LANG_JAVA)
         && chunk.GetType() == E_Token::MEMBER
         && !memcmp(chunk.GetLogText(), "->", 2))
      {
         chunk.SetType(E_Token::LAMBDA);
      }

      // Don't create an entry for whitespace
      if (chunk.GetType() == E_Token::WHITESPACE)
      {
      ...
      else if (chunk.GetType() == E_Token::NL_CONT)
      {
         last_was_tab = chunk.GetAfterTab();
         chunk.SetAfterTab(false);
      }
      ...
      if (chunk.GetType() != E_Token::IGNORED)
      {
         // Issue #1338
      ...
      if (pc->Is(E_Token::NEWLINE)) // || pc->Is(E_Token::COMMENT_MULTI))
      {
      ...
      if (cpd.in_preproc != E_Token::NONE)
      {
         ...
         if (  pc->Is(E_Token::STRING_MULTI)
            && pc->GetParentType() == E_Token::PP_INCLUDE)
         {
         ...
         if (cpd.in_preproc == E_Token::PP_PRAGMA)
         {
         ...
         {
            if (  pc->GetType() < E_Token::PP_DEFINE
               || pc->GetType() > E_Token::PP_OTHER)
            {
               pc->SetType(E_Token::PP_OTHER);
            }
            cpd.in_preproc = pc->GetType();
         }
         else if (cpd.in_preproc == E_Token::PP_IGNORE)
         {
         ...
         else if (  cpd.in_preproc == E_Token::PP_DEFINE
      ...
      else
      {
         ...
         if (  pc->Is(E_Token::POUND)
            && (  rprev->IsNullChunk()
               || rprev->Is(E_Token::NEWLINE)))
         {
            pc->SetType(E_Token::PREPROC);
            pc->SetFlagBits(PCF_IN_PREPROC);
