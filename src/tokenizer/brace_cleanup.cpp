...
struct BraceState
{
   ...
   E_Token           in_preproc = E_Token::NONE;
   int               pp_level   = 0;
   bool              consumed   = false;
};
 ...
 * decremented.
 *
 * - if the TOS is now VBRACE, insert a E_Token::VBRACE_CLOSE and recurse.
...
/**
 * pc is a E_Token::WHILE.
 * Scan backwards to see if we find a brace/vbrace with the parent set to E_Token::DO
 */
...
static size_t preproc_start(BraceState &braceState, ParsingFrame &frm, Chunk *pc)
{
   ...
   if (braceState.in_preproc != E_Token::PP_DEFINE)
   {
   ...
   frm.top().SetOpenToken(E_Token::PP_DEFINE);
...
void brace_cleanup()
{
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      if (  braceState.in_preproc != E_Token::NONE
         && !pc->TestFlags(PCF_IN_PREPROC))
      {
         ...
         braceState.in_preproc = E_Token::NONE;
      }
      ...
      if (  !pc->IsCommentOrNewline()
         && !pc->Is(E_Token::ATTRIBUTE)
         && !pc->Is(E_Token::IGNORED)            // Issue #2279
         && (  braceState.in_preproc == E_Token::PP_DEFINE
            || braceState.in_preproc == E_Token::NONE))
      {
         braceState.consumed = false;
         parse_cleanup(braceState, frm, pc);
         print_stack(LBCSAFTER, (pc->Is(E_Token::VBRACE_CLOSE)) ? "Virt-}\n" : pc->GetText().GetLogText(), frm);
      }
      pc = pc->GetNext();
   }
} // brace_cleanup
...
static bool maybe_while_of_do(Chunk *pc)
{
   ...
   if (  (  prev->Is(E_Token::VBRACE_CLOSE)
         || prev->Is(E_Token::BRACE_CLOSE))
      && prev->GetParentType() == E_Token::DO)
   {
      return(true);
   }
   return(false);
...
static void parse_cleanup(BraceState &braceState, ParsingFrame &frm, Chunk *pc)
{
   ...
   if (  (  frm.GetStmtCount() == 0
         || frm.GetExprCount() == 0)
      && !pc->IsSemicolon()
      && pc->IsNot(E_Token::BRACE_CLOSE)
      && pc->IsNot(E_Token::VBRACE_CLOSE)
      && !pc->IsString(")")
      && !pc->IsString("]"))
   {
   ...
   if (frm.GetSParenCount() > 0)
   {
      ...
      {
         if (frm.at(tmp).GetOpenToken() == E_Token::FOR)
         {
      ...
      if (  pc->Is(E_Token::SEMICOLON)
         && frm.size() > 2
         && frm.prev().GetOpenToken() == E_Token::FOR)
      {
         pc->SetParentType(E_Token::FOR);
      }
   }

   // Check the progression of complex statements
   if (  frm.top().GetStage() != E_BraceStage::NONE
      && !pc->Is(E_Token::AUTORELEASEPOOL)
    ...
    */
   if (frm.top().GetOpenToken() == E_Token::VBRACE_OPEN)
   {
      if (pc->IsSemicolon())
      {
   ...
   {
      // Change E_Token::PAREN_CLOSE into E_Token::SPAREN_CLOSE or E_Token::FPAREN_CLOSE
      if (  pc->Is(E_Token::PAREN_CLOSE)
         && (  (frm.top().GetOpenToken() == E_Token::FPAREN_OPEN)
            || (frm.top().GetOpenToken() == E_Token::SPAREN_OPEN)))
      {
         // TODO: fix enum hack
      ...
      else
      {
         ...
         if (  language_is_set(lang_flag_e::LANG_D)
            ...
            && frm.top().GetOpenChunk()->Is(E_Token::VBRACE_OPEN))
         {
   ...
   if (frm.top().GetStage() == E_BraceStage::WOD_SEMI)
   {
      ...
      else
      {
         ...
         {
            braceState.consumed = true;
            pc->SetParentType(E_Token::WHILE_OF_DO);
         }
         else
         {
      ...
      if (prev->IsNotNullChunk())
      {
            ...
            || pc->Is(E_Token::SPAREN_OPEN))
         {
            ...
            if (  prev->Is(E_Token::IF)
               || prev->Is(E_Token::CONSTEXPR)
               || prev->Is(E_Token::ELSEIF)
               || prev->Is(E_Token::WHILE)
               || prev->Is(E_Token::WHILE_OF_DO)
               || prev->Is(E_Token::DO)
               || prev->Is(E_Token::FOR)
               || prev->Is(E_Token::SWITCH)
               || prev->Is(E_Token::CATCH)
               || prev->Is(E_Token::SYNCHRONIZED)
               || prev->Is(E_Token::D_VERSION)
               || prev->Is(E_Token::D_VERSION_IF)
               || prev->Is(E_Token::D_SCOPE)
               || prev->Is(E_Token::D_SCOPE_IF))
            {
               pc->SetType(E_Token::SPAREN_OPEN);
               parentType = frm.top().GetOpenToken();
               frm.SetSParenCount(frm.GetSParenCount() + 1);
            }
            else if (prev->Is(E_Token::FUNCTION))
            {
               pc->SetType(E_Token::FPAREN_OPEN);
               parentType = E_Token::FUNCTION;
            }
            // NS_ENUM and NS_OPTIONS are followed by a (type, name) pair
            else if (  prev->Is(E_Token::ENUM)
                    && language_is_set(lang_flag_e::LANG_OC))
            {
               // Treat both as E_Token::ENUM since the syntax is identical
               pc->SetType(E_Token::FPAREN_OPEN);
               parentType = E_Token::ENUM;
            }
            else if (prev->Is(E_Token::DECLSPEC))  // Issue 1289
            {
               parentType = E_Token::DECLSPEC;
            }
            // else: no need to set parent
         }
         else  // must be E_Token::BRACE_OPEN
         {
            ...
            else if (  prev->Is(E_Token::ASSIGN)
                    && (prev->GetText()[0] == '='))
            {
               parentType = E_Token::ASSIGN;
            }
            else if (  prev->Is(E_Token::RETURN)
                    && language_is_set(lang_flag_e::LANG_CPP))
            {
               parentType = E_Token::RETURN;
            }
            // Carry through E_Token::ENUM parent in NS_ENUM (type, name) {
            // only to help the vim command }
            else if (  prev->Is(E_Token::FPAREN_CLOSE)
                    && language_is_set(lang_flag_e::LANG_OC)
    ...
    * Note that E_Token::VBRACE_OPEN has already been handled.
    */
      ...
      || pc->Is(E_Token::SQUARE_OPEN))
   {
         ...
         || pc->Is(E_Token::MACRO_OPEN))
      {
         ...
         if (pc->GetParentType() == E_Token::NAMESPACE)
         {
            LOG_FMT(LBCSPOP, "%s(%d): parent type is NAMESPACE\n",
                    __func__, __LINE__);
            Chunk *tmp = frm.top().GetOpenChunk();

            if (tmp->GetParentType() == E_Token::NAMESPACE)
            {
    ...
    * version, E_Token::D_SCOPE_IF
    */
   if (patcls == pattern_class_e::BRACED)
   {
      frm.push(pc, __func__, __LINE__, (pc->Is(E_Token::DO) ? E_BraceStage::BRACE_DO
                    : E_BraceStage::BRACE2));
      // "+ComplexBraced"
   }
   else if (patcls == pattern_class_e::PBRACED)
   {
      ...
      if (  pc->Is(E_Token::WHILE)
         && maybe_while_of_do(pc))
      {
         pc->SetType(E_Token::WHILE_OF_DO);
    ...
    *  - after '(' that has a parent type of E_Token::FOR
    */
   if (  pc->Is(E_Token::SQUARE_OPEN)
      ...
      || (  pc->IsSemicolon()
         && frm.top().GetOpenToken() != E_Token::PAREN_OPEN
         && frm.top().GetOpenToken() != E_Token::FPAREN_OPEN
         && frm.top().GetOpenToken() != E_Token::SPAREN_OPEN)
      || pc->Is(E_Token::MACRO))                         // Issue #2742
   {
...
static bool check_complex_statements(ParsingFrame &frm, Chunk *pc, const BraceState &braceState)
{
   ...
   {
      frm.top().SetStage(pc->IsNot(E_Token::PAREN_OPEN)
                        ? E_BraceStage::BRACE2
                        : E_BraceStage::PAREN1);
   ...
   // Check for E_Token::ELSE after E_Token::IF
   while (frm.top().GetStage() == E_BraceStage::ELSE)
   {
      if (pc->Is(E_Token::ELSE))
      {
         // Replace E_Token::IF with E_Token::ELSE on the stack & we are done
         frm.top().SetOpenToken(E_Token::ELSE);
         frm.top().SetStage(E_BraceStage::ELSEIF);
         print_stack(LBCSSWAP, "=Swap   ", frm);

         return(true);
      }
      // Remove the E_Token::IF and close the statement
      LOG_FMT(LBCSPOP, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
              __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
      frm.pop(__func__, __LINE__, pc);
   ...
   // Check for E_Token::IF after E_Token::ELSE
   if (frm.top().GetStage() == E_BraceStage::ELSEIF)
   {
      ...
      {
         // Replace E_Token::ELSE with E_Token::IF
         pc->SetType(E_Token::ELSEIF);
         frm.top().SetOpenToken(E_Token::ELSEIF);
   ...
   // Check for E_Token::CATCH or E_Token::FINALLY after E_Token::TRY or E_Token::CATCH
   while (frm.top().GetStage() == E_BraceStage::CATCH)
   {
      if (  pc->Is(E_Token::CATCH)
         || pc->Is(E_Token::FINALLY))
      {
         // Replace E_Token::TRY with E_Token::CATCH or E_Token::FINALLY on the stack & we are done
         frm.top().SetOpenToken(pc->GetType());

         if (  language_is_set(lang_flag_e::LANG_CS)
            || language_is_set(lang_flag_e::LANG_VALA))
         {
            frm.top().SetStage((pc->Is(E_Token::CATCH)) ? E_BraceStage::CATCH_WHEN : E_BraceStage::BRACE2);
         }
         else
         {
            ...
            frm.top().SetStage((pc->Is(E_Token::CATCH)) ? E_BraceStage::PAREN1 : E_BraceStage::BRACE2);
         }
         print_stack(LBCSSWAP, "=Swap   ", frm);

         return(true);
      }
      // Remove the E_Token::TRY and close the statement
      LOG_FMT(LBCSPOP, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
              __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
      frm.pop(__func__, __LINE__, pc);
   ...
   // Check for optional parenthesis and optional E_Token::WHEN after E_Token::CATCH
   if (frm.top().GetStage() == E_BraceStage::CATCH_WHEN)
   {
      if (pc->Is(E_Token::PAREN_OPEN)) // this is for the paren after "catch"
      {
         // Replace E_Token::PAREN_OPEN with E_Token::SPAREN_OPEN
         pc->SetType(E_Token::SPAREN_OPEN);
         frm.top().SetOpenToken(pc->GetType());
         frm.top().SetStage(E_BraceStage::PAREN1);
   ...
   // Check for E_Token::WHILE after the E_Token::DO
   if (frm.top().GetStage() == E_BraceStage::WHILE)
   {
      if (pc->Is(E_Token::WHILE))
      {
         pc->SetType(E_Token::WHILE_OF_DO);
         frm.top().SetOpenToken(E_Token::WHILE_OF_DO);
   ...
   // Insert a E_Token::VBRACE_OPEN, if needed
   ...
   // Check for "constexpr" after E_Token::IF or E_Token::ELSEIF
   if (  frm.top().GetStage() == E_BraceStage::PAREN1
      && (  frm.top().GetOpenToken() == E_Token::IF
         || frm.top().GetOpenToken() == E_Token::ELSEIF)
      && pc->Is(E_Token::CONSTEXPR))
   {
      return(false);
   }

   // Verify open parenthesis in complex statement
   if (  pc->IsNot(E_Token::PAREN_OPEN)
...
static bool handle_complex_close(ParsingFrame &frm, Chunk *pc, const BraceState &braceState)
{
   ...
   if (frm.top().GetStage() == E_BraceStage::PAREN1)
   {
      if (pc->GetNext()->GetType() == E_Token::WHEN)
      {
         frm.top().SetOpenToken(pc->GetType());
   ...
   else if (frm.top().GetStage() == E_BraceStage::BRACE2)
   {
      ...
      if (  (frm.top().GetOpenToken() == E_Token::IF)
         || (frm.top().GetOpenToken() == E_Token::ELSEIF))
      {
         frm.top().SetStage(E_BraceStage::ELSE);

         // If the next chunk isn't E_Token::ELSE, close the statement
         Chunk *next = pc->GetNextNcNnl();

         if (  next->IsNullChunk()
            || next->IsNot(E_Token::ELSE))
         {
            LOG_FMT(LBCSPOP, "%s(%d): no E_Token::ELSE, pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            frm.pop(__func__, __LINE__, pc);
      ...
      else if (  (frm.top().GetOpenToken() == E_Token::TRY)
              || (frm.top().GetOpenToken() == E_Token::CATCH))
      {
         frm.top().SetStage(E_BraceStage::CATCH);

         // If the next chunk isn't E_Token::CATCH or E_Token::FINALLY, close the statement
         Chunk *next = pc->GetNextNcNnl();

         if (  next->IsNot(E_Token::CATCH)
            && next->IsNot(E_Token::FINALLY))
         {
            LOG_FMT(LBCSPOP, "%s(%d): pc orig line is %zu, orig col is %zu, text is '%s', type is %s\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            frm.pop(__func__, __LINE__, pc);
...
static void mark_namespace(Chunk *pns)
{
   ...
   {
      is_using = true;
      pns->SetParentType(E_Token::USING);
   }
   pc = pns->GetNextNcNnl();

   while (pc->IsNotNullChunk())
   {
      pc->SetParentType(E_Token::NAMESPACE);
      ...
      if (pc->IsNot(E_Token::BRACE_OPEN))
      {
         if (pc->Is(E_Token::SEMICOLON))
         {
            if (is_using)
            {
               pc->SetParentType(E_Token::USING);
            }
            return;
         }
         pc = pc->GetNextNcNnl();
      ...
      flag_parens(pc, PCF_IN_NAMESPACE, E_Token::NONE, E_Token::NAMESPACE, false);
      return;
   }
} // mark_namespace
...
static Chunk *insert_vbrace(Chunk *pc, bool after, const ParsingFrame &frm)
{
   ...
   if (after)
   {
      ...
      chunk.SetType(E_Token::VBRACE_CLOSE);
   ...
   {
      if (ref->Is(E_Token::PREPROC_BODY))
      {
      ...
      else
      {
         ...
         if (ref->Is(E_Token::COMMENT)) // Issue #3034
         {
            ref = ref->GetNextNc();
         }
      }
   ...
   chunk.SetType(E_Token::VBRACE_OPEN);
...
bool close_statement(ParsingFrame &frm, Chunk *pc, const BraceState &braceState)
{
   ...
   /*
    * Insert a E_Token::VBRACE_CLOSE, if needed:
    * If we are in a virtual brace and we are not ON a E_Token::VBRACE_CLOSE add one
    */
   Chunk *vbc = pc;
