...
/*
 * Issue #3558
 * will be called if a ? (E_Token::QUESTION) chunk is encountered
 * return the chunk colon if found or Chunk::NullChunkPtr
 * if a ; (E_Token::SEMI_COLON) chunk is found
 *
 ...
 * Sibling ternaries should not mark subsequent OC selector colons as E_Token::COND_COLON.
 */
Chunk *search_for_colon(Chunk *pc_question, int depth, bool is_sibling_ternary = false)
{
   ...
   if (pc2->Is(E_Token::COLON))
   {
      return(pc2);
   }

   // examine the next tokens, look for E2, E3, COLON, might be for a next E_Token::QUESTION
   while (pc2->IsNotNullChunk())
   {
      ...
      if (  (  pc2->Is(E_Token::SEMICOLON)
            || (  pc2->Is(E_Token::PAREN_CLOSE)
               && (pc_question->GetLevel() == pc2->GetLevel() + 1))
            || pc2->Is(E_Token::COMMA))
      ...
      else if (pc2->Is(E_Token::COMMA))
      {
         // TODO: is it necessary?
      }
      else if (pc2->Is(E_Token::QUESTION))
      {
         ...
         if (colon_found && colon_after_colon_found)
         {
            ...
            // OC selector colons as E_Token::COND_COLON.
         ...
         else
         {
            ...
            if (next_after_q->Is(E_Token::COLON))
            {
               // This is an elvis operator (?:). Mark the colon as E_Token::COND_COLON
               ...
               next_after_q->SetType(E_Token::COND_COLON);
      ...
      else if (pc2->Is(E_Token::COND_COLON))
      {
         LOG_FMT(LCOMBINE, "%s(%d): orig line is %zu, orig col is %zu, level is %zu, text is '%s'\n",
                 __func__, __LINE__, pc2->GetOrigLine(), pc2->GetOrigCol(), pc2->GetLevel(), pc2->GetLogText());
              ...
              && brace_depth <= 0)  // Test #51012: Don't mark dictionary colons as ternary colons
      {
         ...
         if (colon_found)
         {
            ...
            if (prev->Is(E_Token::WORD) || prev->Is(E_Token::TYPE) || prev->Is(E_Token::OC_MSG_NAME))
            {
         ...
         {
            // There can only be another E_Token::COND_COLON if there is more than 1 E_Token::QUESTION (ie. depth > 0)
            // BUT: If this is a sibling ternary (Test #51008), we should NOT mark subsequent colons
            // as E_Token::COND_COLON because they are OC selector colons, not nested ternary colons.
            pc2->SetType(E_Token::COND_COLON);
            return(pc2);
         }
         else if (!colon_found)
         {
      ...
      else if (pc2->Is(E_Token::SQUARE_OPEN))
      {
         square_bracket_depth++;
      }
      else if (pc2->Is(E_Token::SQUARE_CLOSE))
      {
         square_bracket_depth--;
      }
      // Test #51012: Track OC dictionary @{...} brace depth
      // Check for E_Token::OC_AT preceding the brace to detect OC dictionary literals
      // Note: Parent type may not be set yet during search_for_colon, so check previous token
      else if (pc2->Is(E_Token::BRACE_OPEN))
      {
         Chunk *prev = pc2->GetPrevNcNnl();

         if (prev->Is(E_Token::OC_AT))
         {
            brace_depth++;
         }
      }
      else if (pc2->Is(E_Token::BRACE_CLOSE))
      {
