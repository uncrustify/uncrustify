...
bool can_be_full_param(Chunk *start, Chunk *end)
{
        ...
        pc = pc->GetNextNcNnlNpp(E_Scope::PREPROC))
   {
      ...
      if (  pc->Is(E_Token::CT_QUALIFIER)
         || pc->Is(E_Token::CT_STRUCT)
         || pc->Is(E_Token::CT_ENUM)
         || pc->Is(E_Token::CT_UNION)
      ...
      else if (pc->Is(E_Token::CT_ANGLE_OPEN))
      {
      ...
      else if (pc->Is(E_Token::CT_ELLIPSIS))
      {
              ...
              && pc->Is(E_Token::CT_PAREN_OPEN))
      {
         ...
         if (tmp2->Is(E_Token::CT_QUALIFIER))
         {
...
bool chunk_ends_type(Chunk *start)
{
   ...
   for ( ; pc->IsNotNullChunk(); pc = pc->GetPrevNcNnlNi()) // Issue #2279
   {
      ...
      if (  (  pc->IsSemicolon()
         ...
         || pc->Is(E_Token::CT_MACRO)
         || pc->Is(E_Token::CT_PP_IF)
         || pc->Is(E_Token::CT_PP_ELSE)
         || pc->Is(E_Token::CT_PP_ENDIF)
         || pc->GetParentType() == E_Token::CT_PP_INCLUDE                       // Issue #3233
...
Chunk *get_d_template_types(ChunkStack &cs, Chunk *open_paren)
{
         ...
         && tmp->GetLevel() > open_paren->GetLevel())
   {
      ...
      else if (tmp->Is(E_Token::CT_COMMA))
      {
         maybe_type = true;
      }
   ...
   if (pc->IsNotNullChunk())
   {
              ...
              && pc->GetPrev()->IsTypeDefinition())               // Issue # 2640
      {
         pc->SetType(E_Token::CT_PTR_TYPE);
              ...
              && pc->GetPrev()->IsNot(E_Token::CT_SQUARE_OPEN))            // Issue # 2166
      {
