...
Chunk *skip_align(Chunk *start)
{
   ...
   if (pc->Is(E_Token::ALIGN))
   {
      ...
      {
         pc = pc->GetNextType(E_Token::PAREN_CLOSE, pc->GetLevel());
...
{
   if (ang_close->Is(E_Token::ANGLE_CLOSE))
   {
      Chunk *pc = ang_close->GetPrevType(E_Token::ANGLE_OPEN, ang_close->GetLevel());
...
{
   if (  ary_def->Is(E_Token::SQUARE_OPEN)
      || ary_def->Is(E_Token::TSQUARE))
   {
...
Chunk *skip_attribute(Chunk *attr)
{
   ...
   while (pc->Is(E_Token::ATTRIBUTE))
   {
      pc = pc->GetNextNcNnl();

      if (pc->Is(E_Token::FPAREN_OPEN))
      {
         pc = pc->GetNextType(E_Token::FPAREN_CLOSE, pc->GetLevel());
      }
   }
   return(pc);
}
...
Chunk *skip_attribute_prev(Chunk *fp_close)
{
   ...
   while (true)
   {
      if (  pc->Is(E_Token::FPAREN_CLOSE)
         && pc->GetParentType() == E_Token::ATTRIBUTE)
      {
         pc = pc->GetPrevType(E_Token::ATTRIBUTE, pc->GetLevel());
      }
      else if (pc->IsNot(E_Token::ATTRIBUTE))
      {
         break;
      }
