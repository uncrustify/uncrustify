...
static bool adj_tokens_match_qualified_identifier_pattern(Chunk *prev, Chunk *next)
{
      ...
      && next->IsNotNullChunk())
   {
      ...
      case E_Token::ANGLE_CLOSE:
          ...
          */
         return(next_token_type == E_Token::DC_MEMBER);
      ...
      case E_Token::DC_MEMBER:
          ...
          * type
          */
         return(  next_token_type == E_Token::TYPE
               || next_token_type == E_Token::WORD);
      ...
      case E_Token::WORD:
          ...
          * - a double colon ("::")
          */
         return(  next_token_type == E_Token::ANGLE_OPEN
               || next_token_type == E_Token::DC_MEMBER);
      ...
      && next->IsNotNullChunk())
   {
      ...
      case E_Token::ANGLE_CLOSE:
         ...
         return(  next->IsPointerOrReference()
               || next_token_type == E_Token::DC_MEMBER
               || next_token_type == E_Token::QUALIFIER
               || next_token_type == E_Token::WORD);
      ...
      case E_Token::BYREF:
          ...
          */
         return(next_token_type == E_Token::WORD);
      ...
      case E_Token::PAREN_OPEN:
         ...
         return(next_token_type == E_Token::PAREN_CLOSE);

      case E_Token::PTR_TYPE:
         /**
      ...
      case E_Token::TYPE:
          ...
          */
         return(  next->IsPointerOrReference()
               || next_token_type == E_Token::ANGLE_OPEN
               || next_token_type == E_Token::DC_MEMBER
               || next_token_type == E_Token::QUALIFIER
               || next_token_type == E_Token::WORD);
      ...
      case E_Token::WORD:
          ...
          */
         return(  next_token_type == E_Token::ANGLE_OPEN
               || next_token_type == E_Token::ASSIGN
               || next_token_type == E_Token::BRACE_OPEN
               || next_token_type == E_Token::DC_MEMBER
               || next_token_type == E_Token::PAREN_OPEN
               || next_token_type == E_Token::SQUARE_OPEN
               || next_token_type == E_Token::TSQUARE);
...
static bool chunk_is_macro_reference(Chunk *pc)
{
      ...
      && !pc->TestFlags(PCF_IN_PREPROC))
   {
      while (next->IsNotNullChunk())
      {
         ...
         next = next->GetNextType(E_Token::MACRO);
      }
   }
   return(false);
...
static std::pair<Chunk *, Chunk *> match_qualified_identifier(Chunk *pc)
{
   ...
   {
      auto *double_colon = start->GetNextType(E_Token::DC_MEMBER);
...
static std::tuple<Chunk *, Chunk *, Chunk *> match_variable(Chunk *pc, std::size_t level)
{
   ...
   /**
    * a forward search starting at the chunk under test will fail if two consecutive chunks marked as E_Token::WORD
   ...
   if (  identifier->IsNotNullChunk()
      && start->IsNotNullChunk()
      && (  end->IsNotNullChunk()
         || identifier->GetPrevNcNnlNi()->Is(E_Token::WORD)))
   {
...
static std::pair<Chunk *, Chunk *> match_variable_end(Chunk *pc, std::size_t level)
{
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      if (  next->IsNot(E_Token::COMMA)
         && next->IsNot(E_Token::FPAREN_CLOSE)
         && !next->IsSemicolon()
...
bool EnumStructUnionParser::enum_base_detected() const
{
   ...
   return(m_chunk_map.find(E_Token::BIT_COLON) != m_chunk_map.cend());
...
Chunk *EnumStructUnionParser::get_body_end() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::BRACE_CLOSE);
...
Chunk *EnumStructUnionParser::get_body_start() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::BRACE_OPEN);
...
Chunk *EnumStructUnionParser::get_enum_base_start() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::BIT_COLON);
...
Chunk *EnumStructUnionParser::get_first_top_level_comma() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::COMMA);
...
Chunk *EnumStructUnionParser::get_inheritance_end() const
{
   ...
   if (inheritance_start->IsNotNullChunk())
   {
      ...
      {
         brace_open = inheritance_start->GetNextType(E_Token::BRACE_OPEN, m_start->GetLevel(), E_Scope::ALL);
      }
   }
   return(brace_open);
...
Chunk *EnumStructUnionParser::get_inheritance_start() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::COLON);
...
std::map<std::size_t, Chunk *> EnumStructUnionParser::get_question_operators() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::QUESTION);
...
Chunk *EnumStructUnionParser::get_template_end() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::ANGLE_CLOSE);
...
Chunk *EnumStructUnionParser::get_template_start() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::ANGLE_OPEN);
...
Chunk *EnumStructUnionParser::get_where_end() const
{
   ...
   if (where_start->IsNotNullChunk())
   {
      ...
      if (brace_open->IsNullChunk())
      {
         brace_open = where_start->GetNextType(E_Token::BRACE_OPEN, m_start->GetLevel(), E_Scope::ALL);
      }
   }
   return(brace_open);
...
Chunk *EnumStructUnionParser::get_where_start() const
{
   ...
   auto &&it_token_chunk_map_pair = m_chunk_map.find(E_Token::WHERE);
...
bool EnumStructUnionParser::inheritance_detected() const
{
   ...
   return(m_chunk_map.find(E_Token::COLON) != m_chunk_map.cend());
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      if (next->Is(E_Token::DC_MEMBER))
      {
         ...
         if (pc->Is(E_Token::WORD))
         {
             ...
             * to change chunks marked E_Token::WORD to E_Token::TYPE if followed by a scope-
             * resolution operator; if a chunk marked E_Token::WORD is followed by a set
      ...
      else if (  (  next->Is(E_Token::BRACE_OPEN)
...
void EnumStructUnionParser::mark_braces(Chunk *brace_open)
{
   ...
   else if (m_start->Is(E_Token::STRUCT))
   {
      flags = PCF_IN_STRUCT;
   }
   /**
...
void EnumStructUnionParser::mark_class_colon(Chunk *colon)
{
   ...
   colon->SetType(E_Token::CLASS_COLON);
...
{
   colon->SetType(E_Token::COND_COLON);
      ...
      && m_start->IsClassOrStruct())
   {
      ...
      for (auto *prev = body_start; next != body_end; prev = next)
      {
         ...
         if (  std::strcmp(prev->GetLogText(), name) == 0
            ...
            && next->IsParenOpen())
         {
            prev->SetType(E_Token::FUNC_CLASS_DEF);
...
void EnumStructUnionParser::mark_enum_integral_type(Chunk *colon)
{
   ...
   colon->SetType(E_Token::ENUM_COLON);
   colon->SetParentType(m_start->GetType());
         ...
         && !pc->IsSemicolon())
   {
       ...
       *       class is improved such that PCF_VAR_TYPE is not set for these chunks
       */
      if (pc->IsNot(E_Token::DC_MEMBER))                             // Issue #3198
      {
         pc->ResetFlagBits(PCF_VAR_TYPE);
         pc->SetType(E_Token::TYPE);
...
void EnumStructUnionParser::mark_extracorporeal_lvalues()
{
    ...
    */
   if (next->GetParentType() == E_Token::TEMPLATE)
   {
      while (true)
      {
         prev = next->GetPrevNcNnlNi();
   ...
   while (next != m_end)
   {
      ...
      else if (  (  next->Is(E_Token::ASSIGN)
...
void EnumStructUnionParser::mark_pointer_types(Chunk *pc)
{
   ...
   if (pc->Is(E_Token::WORD))
   {
      do
      {
         // TODO: should there be a E_Token::BYREF_TYPE?
         pc = pc->GetPrevNcNnlNi();
...
void EnumStructUnionParser::mark_where_colon(Chunk *colon)
{
   ...
   colon->SetType(E_Token::WHERE_COLON);
   colon->SetParentType(m_start->GetType());
...
void EnumStructUnionParser::parse(Chunk *pc)
{
    ...
    */
   if (  m_start->GetParentType() == E_Token::C_CAST
   ...
   if (!is_within_inheritance_list(pc))
   {
      ...
      else
      {
         ...
         if (next->IsNot(E_Token::DC_MEMBER))
         {
             ...
             * bracket should be preceded by a E_Token::WORD token and we should have
   ...
   if (brace_close->IsNotNullChunk())
   {
         ...
         && prev->IsParenClose())
      {
         ...
         if (paren_open->IsNotNullChunk())
         {
            ...
            is_potential_function_definition = (  (  identifier->Is(E_Token::FUNCTION)
                                                  || identifier->Is(E_Token::FUNC_DEF)
                                                  || identifier->Is(E_Token::WORD))
                                               && type != identifier);
         }
      }
...
void EnumStructUnionParser::record_question_operator(Chunk *question)
{
   ...
   if (question->Is(E_Token::QUESTION))
   {
      std::size_t index = m_chunk_map[E_Token::QUESTION].size();

      m_chunk_map[E_Token::QUESTION][index] = question;
   }
...
void EnumStructUnionParser::record_top_level_comma(Chunk *comma)
{
   ...
   {
      std::size_t index = m_chunk_map[E_Token::COMMA].size();

      m_chunk_map[E_Token::COMMA][index] = comma;
   }
...
void EnumStructUnionParser::set_body_end(Chunk *body_end)
{
   ...
   if (body_end->Is(E_Token::BRACE_CLOSE))
   {
      m_chunk_map[E_Token::BRACE_CLOSE][0] = body_end;
   }
...
void EnumStructUnionParser::set_body_start(Chunk *body_start)
{
   ...
   if (body_start->Is(E_Token::BRACE_OPEN))
   {
      m_chunk_map[E_Token::BRACE_OPEN][0] = body_start;
   }
...
void EnumStructUnionParser::set_enum_base_start(Chunk *enum_base_start)
{
   ...
   {
      m_chunk_map[E_Token::BIT_COLON][0] = enum_base_start;
   }
...
void EnumStructUnionParser::set_inheritance_start(Chunk *inheritance_start)
{
   ...
   {
      m_chunk_map[E_Token::COLON][0] = inheritance_start;
   }
...
void EnumStructUnionParser::set_template_end(Chunk *template_end)
{
   ...
   if (template_end->Is(E_Token::ANGLE_CLOSE))
   {
      m_chunk_map[E_Token::ANGLE_CLOSE][0] = template_end;
   }
...
void EnumStructUnionParser::set_template_start(Chunk *template_start)
{
   ...
   if (template_start->Is(E_Token::ANGLE_OPEN))
   {
      m_chunk_map[E_Token::ANGLE_OPEN][0] = template_start;
   }
...
void EnumStructUnionParser::set_where_end(Chunk *where_end)
{
   ...
   if (where_end->Is(E_Token::BRACE_OPEN))
   {
      m_chunk_map[E_Token::WHERE][0] = where_end;
   }
...
void EnumStructUnionParser::set_where_start(Chunk *where_start)
{
   ...
   if (where_start->Is(E_Token::WHERE))
   {
      m_chunk_map[E_Token::WHERE][0] = where_start;
   }
...
void EnumStructUnionParser::try_post_identify_macro_calls()
{
      ...
      && type_identified())
   {
       ...
       * macro calls and mark them as E_Token::MACRO_FUNC_CALL. The reason for doing
      ...
      do
      {
         if (  !chunk_is_between(prev, inheritance_start, body_start)
            && (  prev->Is(E_Token::WORD)
               || prev->Is(E_Token::FUNCTION)
               || prev->Is(E_Token::FUNC_DEF))
            && !prev->GetFlags().test_any(PCF_VAR_DEF | PCF_VAR_1ST | PCF_VAR_INLINE)
            && prev->GetLevel() == m_start->GetLevel())
         {
            if (pc->IsParenOpen())
            {
               ...
               {
                  paren_open->SetType(E_Token::FPAREN_OPEN);
                  paren_open->SetParentType(E_Token::MACRO_FUNC_CALL);
                  paren_close->SetType(E_Token::FPAREN_CLOSE);
                  paren_close->SetParentType(E_Token::MACRO_FUNC_CALL);
                  prev->SetType(E_Token::MACRO_FUNC_CALL);
               }
            }
         }
         prev = pc;
   ...
   if (pc->IsNullChunk())
   {
              ...
              && m_end->GetPrevNcNnlNi() == next_next)
      {
         ...
         if (  chunk_is_macro_reference(next)
            || m_start->GetParentType() == E_Token::TEMPLATE)
         {
            pc = m_end;
         }
         else
