...
void enum_cleanup()
{
   ...
   {
      if (  pc->GetParentType() == E_Token::ENUM
         && pc->Is(E_Token::BRACE_CLOSE))
      {
         ...
         if (prev->IsNotNullChunk())
         {
            ...
            {
               if (prev->Is(E_Token::BRACE_OPEN))                // Issue #2902
               {
                  // nothing between E_Token::BRACE_OPEN and E_Token::BRACE_CLOSE
               }
               else
               {
                  log_rule_B("mod_enum_last_comma");
