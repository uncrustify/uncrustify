...
static bool can_remove_braces(Chunk *bopen)
{
   ...
   if (bopen->GetFlags().test(PCF_IN_PREPROC))
   {
      return(false);
   }
...
static void convert_brace(Chunk *br)
{
   ...
   if (  br == nullptr
      || br->GetFlags().test(PCF_KEEP_BRACE))
   {
      return;
   }
   Chunk *tmp;
   ...
   if (tmp->IsNewline())
   {
      if (tmp->nl_count > 1)
      {
         if (!br->GetFlags().test(PCF_ONE_LINER)) // Issue #2232
         {
            tmp->nl_count--;
...
static void convert_vbrace_to_brace()
{
   ...
   for (Chunk *pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnl())
   {
      ...
      auto const in_preproc = pc->GetFlags().test(PCF_IN_PREPROC);
            ...
            && (options::mod_full_brace_function() & IARF_ADD)))
      {
         ...
         {
            if (  in_preproc
               && !tmp->GetFlags().test(PCF_IN_PREPROC))
            {
...
static Chunk *mod_case_brace_remove(Chunk *br_open)
{
        ...
        tmp_pc = tmp_pc->GetNextNcNnl(E_Scope::PREPROC))
   {
      if (  tmp_pc->level == (br_open->level + 1)
         && tmp_pc->GetFlags().test(PCF_VAR_DEF))
      {
