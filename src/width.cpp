...
static bool split_line(Chunk *start)
{
   ...
   LOG_FMT(LSPLIT, "   start->GetParentType() %s, (PCF_IN_FCN_DEF is %s), (PCF_IN_FCN_CALL is %s)\n",
           ...
           start->GetFlags().test((PCF_IN_FCN_DEF)) ? "TRUE" : "FALSE",
           start->GetFlags().test((PCF_IN_FCN_CALL)) ? "TRUE" : "FALSE");

   // break at maximum line length if ls_code_width is true
   // Issue #2432
   if (start->GetFlags().test(PCF_ONE_LINER))
   {
   ...
   else if (start->GetFlags().test(PCF_IN_FOR))
   {
    ...
    */
   else if (  start->GetFlags().test(PCF_IN_FCN_DEF)
           || start->GetParentType() == CT_FUNC_PROTO            // Issue #1169
           || (  (start->level == (start->brace_level + 1))
              && start->GetFlags().test(PCF_IN_FCN_CALL)))
   {
    ...
    */
   else if (start->GetFlags().test(PCF_IN_TEMPLATE))
   {
