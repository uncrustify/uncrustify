...
bool complex_nested_conditions(int x, int y, int z) {
	if ((x == y || (y < z && z > 20)) && ((x + y > z * 2) || (x - z < y * 3 && x > 100))) {
		return true;
	}
	return false;
}
...
int main() {
	...
	if (nested_boolean_example(a, b, c, d) && complex_nested_conditions(x, y, z)) {
		printf("Both nested conditions are true.\n");
	}

	if ((a < b && (b == c || d > 100)) || ((a * b - d < c * c + b) && (d != 1))) {
