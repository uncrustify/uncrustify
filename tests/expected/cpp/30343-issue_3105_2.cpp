        ...
        const char *_name)
{
	...
	ret =
	        ...
	        TDEStdAccel::shortcut(TDEStdAccel::ShowMenubar), recvr, slot, parent, _name ? _name : name(
	        ShowMenubar));
...
KProgressBoxDialog::KProgressBoxDialog(TQWidget *parent, const char *name, const TQString &caption,
		...
		KDialogBase(KDialogBase::Plain, caption, KDialogBase::Cancel,
		        ...
		        mMinDuration(
		        2000), d(new KProgressBoxDialogPrivate)
{
...
TDEPopupMenu* KPixmapRegionSelectorWidget::createPopupMenu()
{
	...
	TDEAction *action = new TDEAction(i18n(
	        "&Rotate Clockwise"), "object-rotate-right", 0, TQT_TQOBJECT(this),
	        TQT_SLOT(rotateClockwise()), TQT_TQOBJECT(popup), "rotateclockwise");
	action->plug(popup);

	action =
	        new TDEAction(i18n("Rotate &Counterclockwise"), "object-rotate-left", 0, TQT_TQOBJECT(
	        this), TQT_SLOT(rotateCounterclockwise()), TQT_TQOBJECT(popup), "rotatecounterclockwise");
	action->plug(popup);
	return popup;
}
